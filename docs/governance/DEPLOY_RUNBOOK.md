...
- **CD web rebuild (ADR-0027 Slice 1+):** not on every push. Ordinary `main`
  ...
  **Owner checklist — attended web rebuild**
  ...
  First production attended PASS: Actions
  [32555455704](https://github.com/tahamohamadi-ir/Taha-personal-platform/actions/runs/32555455704)
  (`rebuild_web=true`, LOG-0199). Prior attempt
  [32555108949](https://github.com/tahamohamadi-ir/Taha-personal-platform/actions/runs/32555108949)
  failed on Docker loopback (fixed in #74 / LOG-0198).
...
| Old pre-Compose stack (`taha-prod-*`) | (ops) | **CLOSED** | LOG-0216: no `taha-prod-*` containers; only `taha-cms` running(4). |
...
Suggested CMS image pin: **`116c241`** (LOG-0216). Earlier pins (`65d6c91`, `e2cd1b6`, …) are historical.
Later `main` commits that are docs/smoke-only may lack a matching `taha-cms:<sha>` tag — pin the last green **CMS image** sha.
