...
| Scheduled-publish timer | (ops; `DEBT-0005` code CLOSED) | **CLOSED** | Owner attestation 2026-08-22: `sudo bash infra/deploy/install-scheduled-publish-timer.sh` → `install-scheduled-publish-timer PASS`; `taha-publish-scheduled-content.timer` active (NEXT 2026-08-22 06:41:00 UTC). LOG-0200. CD path optional (`install_scheduled_timer=true`). |
...
6. **Install scheduled-publish timer** (needed for `scheduled` → published).
   ...
   **Production PASS (owner attestation 2026-08-22, LOG-0200):** manual
