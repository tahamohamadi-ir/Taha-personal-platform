...
**UPDATE 2026-08-23 (ADR-0030):** owner gave in-session license acceptance for gsap and authorized motion/gsap/d3/three per `docs/adr/0030-visual-interaction-libraries.md`. three.js ships as a lazy ≤150KB-gzip chunk for the single 3D signature experience (hero constellation, `client:visible`, static-SVG fallback); measured chunk 128.7KB gzip ≤ budget. gsap authorized for narrative scroll interactions (not yet imported in any shipped bundle).

**Verdict: PASS with conditions** — no MISSING/unverifiable licenses; the only items outside the acceptable list are (a) gsap (proprietary — **owner license acceptance recorded 2026-08-23 via ADR-0030**), (b) the LGPL-3.0-or-later component of the sharp Windows binary, and (c) MPL-2.0 lightningcss. Items (b) and (c) are in the build toolchain of the static artifact and are common in practice, but are flagged because they fall outside the policy's pre-approved license list. Re-run this report after any dependency change.
