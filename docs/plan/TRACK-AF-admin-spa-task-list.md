# Track AF — Admin Frontend SPA (`apps/cms/admin-frontend`) — Task List
...
> Read FIRST: `AGENTS.md`, `docs/plan/TRACK-MODE-environment-and-cutover.md`, `docs/plan/TRACK-AB-admin-backend-api-task-list.md` (endpoint contracts are FROZEN there — AF consumes, never redefines), existing `apps/cms/admin-frontend/src/**` (entity registry, api client, ProblemDetails parser, i18n pattern).

## 0. Ownership wall

- WRITE zone: `apps/cms/admin-frontend/**` only. `package.json` edits limited to adding zero runtime deps by default (see per-packet rules); scripts additions allowed.
...
- Local run: mode A1 stack up → `npm ci && npm run dev` inside admin-frontend; login with the local throwaway superuser. Vite proxy for `/api/v1/*` must point at `http://127.0.0.1:18000` — if a dev-proxy config already exists, reuse it; creating one is part of AF-00 allowed edits (`vite.config.*`).

Loop per packet:

```powershell
cd apps/cms/admin-frontend
...
| 5 | Zone purity | diff limited to `admin-frontend/**` | foreign edit |
