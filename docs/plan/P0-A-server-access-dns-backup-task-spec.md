...
7. Preliminary read-only audit found pre-existing Caddy and Docker services. The staging DNS record was then created, but external verification reports a Cloudflare 525 TLS-origin handshake failure. Inventory the current routing and container metadata without reading or logging secrets before any Caddy change; the staging hostname must not accidentally serve the legacy production site.
...
10. Apply ADR-0015 as the smallest reversible repair: back up and validate Caddyfile, add a dedicated staging block that returns 503 with Caddy automatic TLS, reload only after validation, and test direct-origin plus Cloudflare behavior. Do not use `tls internal`, proxy to production, or change Cloudflare zone mode in this slice.
