...
7. Preliminary read-only audit found pre-existing Caddy and Docker services. Before creating the staging DNS record, inventory the current routing and container metadata without reading or logging secrets; the staging hostname must not accidentally serve the legacy production site.
