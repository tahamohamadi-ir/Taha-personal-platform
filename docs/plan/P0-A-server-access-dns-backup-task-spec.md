...
10. ADR-0015 was applied: Caddyfile backup/validation/reload succeeded and external Cloudflare behavior is explicit 503. The direct-origin curl probe returned a TLS internal alert; diagnose that evidence before Full (strict) or a real staging application. Do not use `tls internal`, proxy to production, or change Cloudflare zone mode in this slice.
