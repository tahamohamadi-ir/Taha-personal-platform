...
10. ADR-0015 was applied: Caddyfile backup/validation/reload succeeded and external Cloudflare behavior is explicit 503. Caddy logs show the initial direct-origin probe preceded successful ACME HTTP-01 certificate issuance; one post-issuance direct-origin re-test is pending before Full (strict) or a real staging application. Do not use `tls internal`, proxy to production, or change Cloudflare zone mode in this slice.
