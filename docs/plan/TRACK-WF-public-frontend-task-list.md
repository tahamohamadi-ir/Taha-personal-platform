...
| `apps/cms/**`, `apps/cms/admin-frontend/**` | FORBIDDEN for WF. Data comes from published-only public API; schemas belong to Tracks BK/AB/AF. |
...
| **WF-07A** | Adopt — Home | Map locale index + landing to `HomeTemplate` 8-block narrative (identity→graph list→research fit→journey→projects→publications→previews→CTA); loaders/DTO/slug unchanged; missing module omitted honestly; Light/Dark + FA narrow screenshots | route + Landing files for `{fa,en}/index`, per-family QA spec | family QA PASS; integration grep shows template import | G3/G5 | WF-05,06 |
