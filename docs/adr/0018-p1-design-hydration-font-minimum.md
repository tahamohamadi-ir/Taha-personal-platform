...
- Typography uses self-hosted, minimal-weight fonts chosen against a bilingual
  `fa`/`en`/mixed specimen; the exact family is an owner decision and no font is
...
- The design-system decisions that require owner input (font family, logo asset)
  are explicit `OPEN` items in `PROJECT_MANIFEST.md` rather than guessed.
