...
- Why: the semantic list is complete but static; exploration (pan/zoom/focus/select) needed the enhancement island MASTER-SPEC promised, consuming the related-slugs surface from LOG-0286 instead of inventing links.
- Scope / files: `apps/web/src/components/research/ResearchGraphIsland.tsx` (new), 4 pages (mount only), `apps/web/src/components/research/ResearchGraph.astro` (JSDoc->fromEntries for astro check), new `apps/web/qa/graph-island.spec.mjs`, this entry.
- Commands or actions actually performed: new spec first → FAIL (missing file); implemented → spec PASS; `npm run check` first 35 errors (`Cannot find name` — TS annotations inside Astro markup break the frontmatter scope) → rewrote untyped (fromEntries) → 0 errors 0 warnings; `graph-consumer` then failed ONLY home sections — real defect found: two adjacent slot children without slot-on-Fragment read as unfilled and HomeTemplate skips the block (evidence: mock-build dist lacked `tpl-home__block--graph` while research rendered) → house pattern `<Fragment slot="graph">` on both homes + a regression guard in the island spec → `graph-consumer` PASS 99 checks + `budget` PASS (6 chunks, all <=150KB gzip) + layoutGraph executed for real via esbuild bundle (pinned x40, ring determinism, empty — PASS); snapshot rebuilt clean.
- Verification actually performed and result: embedded above; live click/keyboard Playwright probe ATTEMPTED but the mock build hit 5s fetch timeouts on this loaded desktop (user Chrome GBs resident — verified via process list, left untouched; no leaked processes of mine found) — recorded honestly, not as pass. Static gates fully green; interaction probe rides the repo Playwright suite on a quiet machine/CI.
- Decisions / assumptions: island control labels stay English like the Lightbox precedent (fa review queued, item 5); related links render ONLY endpoint-resolved hrefs with `family:id` text (data, never copy); island is a sibling AFTER the list so the list's zero-href assertion still holds on the extracted section.
- Deferred or risk IDs: G7-island Playwright interaction proof (follow-up, needs quiet machine); none new.
- Rollback / recovery: revert this commit (additive island; list untouched).
- Doctrine compliance: [TDD fail-first gate spec; §7.1 props-in island + shared surface; §7.3 rows 1/2/6/7/8/10 — zero new deps, keyboard parity, ASCII-only, no-JS intact].
...
- Why: owner-queue item (12); any dist-consumer step sequenced after home-cms-build (sitemap/pagefind/health/fingerprint) previously ran against a fail-build-corrupted dist.
- Scope / files: `apps/web/qa/home-cms-build.spec.mjs` (append-only, reuses already-imported `spawnSync`/`rmSync`/`join`), `.github/workflows/ci.yml` (one step), this entry.
- Commands or actions actually performed: `node --check qa/home-cms-build.spec.mjs` (SYNTAX-OK); first spec run FAILED pre-existing (`English home page is missing cms-build-origin snapshot meta`) because the worktree `dist/` was a stale CMS-backed build (`content="cms"`) — rebuilt snapshot via `npm run build` (unset base), re-ran spec → `PASS home-cms-build` + `PASS home-cms-build restore snapshot`; then `node qa/p8-catalog.spec.mjs` → `PASS p8-catalog parity` + `PASS p8-catalog restore snapshot` (proves downstream specs survive).
- Verification actually performed and result: embedded above; LOG ID allocated per docs/README.md §4 (max across all refs = 0281 → 0282).
- Decisions / assumptions: restore uses `spawnSync` (file's existing style) rather than p8's async `runBuild`; no product-code change, QA-hygiene only.
- Deferred or risk IDs: none new; owner queue shrinks to 11 (label dicts, researchFit mapping, lightbox nav, More/ThemeToggle keys, composed-fa review, gsap, crop sign-off, light-art, G7 island, related-slugs, embedded-preview DEFER).
- Rollback / recovery: revert this commit.
- Doctrine compliance: [packet QA TDD order — spec change verified by real fail→pass→downstream run; zone purity — only `apps/web/qa/*` + `.github/workflows/ci.yml`].

## LOG-0281 — 2026-08-27 — Wave 11 (final build wave): AF-02b Timeline screen + WF-CLEAN sweep — ALL TRACK PACKETS CLOSED

- Outcome: Tenth parallel wave (2 agents, disjoint zones): **AF-02b** — TimelineEditorPage composed from the existing four parts: reducer->API wiring map complete (LOAD->listTimeline, ROW_SAVE->updateTimeline with 428 silent-retry-once-fresh-revision then error / 409 conflict dialog, CREATE->createTimeline with after_id+attach, DELETE->deleteTimeline 428-retry/409-DELETE_CONFLICT new action, ORDER_SAVE->reorderTimeline); route /timeline-editor/:locale + nav child; toast sentinels ALL mapped to dictionary keys (9 toasts); +39 keys x2 -> 190==190 parity; qa:tokens guard EXTENDED (TIMELINE_TYPES->type.*, TIMELINE_FIELD_TOKENS->token.*, locale.* namespace enumeration - template-built namespaces can no longer silently 404); reducer repairs: duplicate dead DELETE_SUCCESS case removed + CREATE_ERRORS_CLEAR/DELETE_CONFLICT added (the abort had left stale-tokens-on-cancel and DELETE-409 unhandled despite its own comments). **WF-CLEAN** — 16 files deleted (76,668B): 10 flagged candidates (ResearchCatalog, ArticleCard/ArticleDetail/TagList, About, Downloads, 4 Landing legacy sections) + 6 newly-discovered orphans (patterns/CatalogPage+DetailShell, primitives Kicker/MetaRow/Btn/Chip) — each zero-import verified by path-specific rg (ContactCTA/Chip name-collisions disambiguated) + npx unimported cross-check (0 unimported .astro; 15 .ts resolver false-positives individually verified); empty dirs patterns//primitives/ removed; cvDownloads.ts DownloadFile interface relocated (6-line comment edit, only consumer of deleted Downloads.astro); NOT deleted despite flags: CaseStudyDetail (story branch imports it), Lightbox/HeroSection/FocusStrip/StoryBody/Landing (retained wrapper); depcheck: gsap genuinely unused (OWNER motion-governance decision pending — nothing uninstalled), three/motion false-positives (dynamic import), fonts/tailwind/tooling false-positives; LEGACY-INVENTORY.md ledger with 16 executed rows + retained table. POST-SWEEP gates: 32/32 web specs PASS (incl. final-matrix 279 pass/4 honest skips via documented PREVIEW_URL invocation), check 0/0, build 58p atlas-free, cms 636, admin tsc 0 + qa:tokens 190==190, validator PASS. Commits: AF-02b -> WF-CLEAN.
- Why: Owner-mandated fastest completion; this closes every packet in all four tracks.
- Scope / files: two agents' allowlists + this entry.
- Commands or actions actually performed: final combined verify (matrix, home-composition, web check, admin check+qa:tokens, cms pytest, validator) -> 2 commits -> this entry.
- Verification actually performed and result: embedded — this is the release-candidate state.
- Decisions / assumptions: gsap removal deferred to owner (bounded-asset governance, LOG-0243 ADR-0030 context); Landing wrapper retained; positional orderDirty heuristic fails-safe (extra confirm, never silent loss).
- Deferred or risk IDs: OWNER QUEUE (final): (1) 4 label dictionaries (project_type/role/methods/level — raw source values render); (2) perspectives->researchFit mapping; (3) nav-capable lightbox variant; (4) More/ThemeToggle content.ts keys; (5) AF composed-fa 134+ rows review (AF-07 report table); (6) gsap removal; (7) gateway/landing visual crop sign-off; (8) light-theme art wiring; (9) G7 interactive 2D island (list-first shipped); (10) related-slugs read endpoint; (11) embedded-preview iframe DEFER; (12) OBS-1 home-cms-build CI wiring.
- Doctrine compliance: AF-02b [reducer-first completion, i18n complete-at-commit enforced by extended guard, zero deps]; WF-CLEAN [MODE 7.3 adopt-equals-delete executed, inventory ledger, deletion-with-verification].

## RELEASE STATE (LOG-0281 close)

G9 READY (FINAL-QA-REPORT.md). Web: tokens dual-theme + 8 primitives + 10 content components + 6 templates + 8/8 route families + graph list-first + assets + local Atlas isolated. Admin: 7 screens over 47 OpenAPI paths. CMS: 636 tests, additive migrations 0017-0019 PG-applied. Local A1 stack live. Remaining before production cutover: owner escalation queue above + owner-approved deploy task (backup/rollback/CI/smoke per AGENTS gate).
## LOG-0280 — 2026-08-27 — Wave 10: WF-10 G9 READY + AF-06 previews + AF-07 sweep (AF-02 partial discovered)

- Outcome: Ninth parallel wave (3 agents): **WF-10 (G9 gate)** — full inventory (32 runnable specs + gates) executed sequentially with real outputs: foundation 6/6, adoption 8/8, graph/asset 2/2, CI set 13/13, Playwright 3/3 (mobile-overflow, about-tabs, NEW final-matrix); NEW final-matrix.spec: 283 tests = 279 pass / 4 honest skips (writing detail absent in snapshot - honest) / 0 fail across 246 cells (10 routes+gateway x en/fa x 6 widths x light/dark via data-theme override: zero horizontal overflow, skip-link, single-H1, lang/dir, gateway stays night under theme=light) + 8 zoom-200% cells + 22 JS-disabled cells (nav/text/form/honeypot intact); dist privacy scans ALL ZERO (fixture/unpublished/data-atlas-id/_design/+98/09xx/gmail/tel:/draft/private; raw leftovers classified binary/vendor); perf spot: dist 12.03MB/249 files, largest art fallbacks 508-797KB (<1MB cap), JS 187KB gzip total, three chunk 126KB<=150KB, Pagefind 27p/locale, LCP preload verified; FINAL-QA-REPORT.md committed with per-area tables + **G9 verdict: READY, no blockers**; defects: none in product code (QA-1 self-spec bug fixed; OBS-1: home-cms-build.spec leaves dist broken + not wired CI - flagged; OBS-2..5 Info). CMS-stack e2e SKIPPED honestly (owned by ci-cms.yml, needs disposable CMS). **AF-06** — GraphPreviewPanel pure component (type-grouped weight-sorted <ol> + relations <ul>, unicodeBidi plaintext arrows CSS-mirrored), collapsible drawer from LIVE draft state, honest notices: locale mismatch renders notice not fake-translation, theme dark renders 'تم تیره در ادمین فعال نیست' (admin has no dark tokens - no fake data-theme emitted); +7 keys x2; DEFER recorded: embedded site iframe preview. **AF-07** — sweep: +23 keys x2 (21 timeline.* the AF-02 components reference but BOTH dictionaries lacked - would have rendered raw keys), VersionsPanel 2 hardcoded literals -> dict, **7 mojibake repairs in HomeComposerPage (double-encoded UTF-8 rendering broken glyphs: em-dash + arrows)**, check-graph-tokens.mjs extended with referenced-key coverage (every statically referenced redesign.*/graph.* literal must exist in BOTH dicts) + unused-key INFO; RTL grep whole src = 0; Appendix A audit: 21 entries/26 comparisons ALL byte-exact (ZWNJ verified); FULL composed-fa owner-review table (134 rows) emitted in report (AF-OWNER-REVIEW.md not created - docs forbidden). **DISCOVERY: AF-02 is PARTIAL** - TimelineEditor screen page/route/nav never built (components+reducer+keys only); queued as AF-02b final packet. Combined gates: matrix + admin check 0 + qa:tokens 160==160 + validator PASS. Commits: b029d16 (WF-10) / 46556da (AF-06+07 combined - shared i18n files).
- Why: Owner-mandated fastest path to G9; WF-10 is the release gate.
- Scope / files: three agents' allowlists + this entry. Shared i18n files forced AF-06+07 single commit.
- Commands or actions actually performed: combined verify (matrix re-run, admin check+qa:tokens, validator) -> 2 commits -> this entry.
- Verification actually performed and result: embedded; final-matrix re-run confirmed after commit.
- Decisions / assumptions: G9 READY verdict stands on snapshot-build evidence; CMS-stack e2e + live A1 visual crop sign-off remain owner-side; AF-06 honest notices chosen over fake dark/locale rendering; AF-07 fixed only feature-tree literals (legacy screens 163 fa regions report-only - separate decision).
- Deferred or risk IDs: AF-02b (screen page) next wave; OBS-1 home-cms-build CI wiring; G7 interactive island; related-slugs; light-art wiring; visual crop sign-off; owner dictionaries (project_type/role/methods/level); More/ThemeToggle keys; embedded-preview DEFER; AF composed-fa 134-row owner review.
- Doctrine compliance: WF-10 [skipped!=PASS enforced, evidence tables]; AF-06 [honest-state over invention, parity automated]; AF-07 [i18n complete-at-commit enforced by NEW usage-coverage guard, RTL zero-tolerance, byte-exact audit].
## LOG-0279 — 2026-08-27 — Wave 9 parallel x3: WF-08 graph consumer + WF-09 assets + AF-05 validation UX (G7 list-first landed)

- Outcome: Eighth parallel wave (3 agents): **WF-08** — ResearchGraph.astro semantic list-first (zero-JS <ol>, type Badge, relation peers with ASCII direction glyphs RTL-mirrored, data surfaces for future island; zero invented hrefs - relatedRecords are {family,pkId} unresolvable to URLs, ESCALATION recorded: slug-bearing read needed for real links); research-graph.ts adapter honesty ladder (unset->null snapshot-omit, 404->null absent, transport/5xx->CmsOriginError fail-build per ADR-0027, malformed->null incl. p8-generic-mock trap) + consumer-side parser drops blank-label/orphan/dup-edge rows; wired into BOTH home graph slots (07A reservation consumed) and both research indexes; snapshot build omits entirely, CMS-mocked build renders per-locale payloads; 93-check spec; **G7 NOTE: 2D interactive island NOT built in this pass - list-first only; interactive island = explicit remaining packet** (semantic list satisfies no-JS; island is enhancement). **WF-09** — generator script resolves sharp from astro's tree (no package.json edit), re-verifies 11 masters against SHA256SUMS before reading pixels (tamper abort), 77 derivatives 8.52MB (AVIF q50/WebP q78/~800-1600 clamped no-upscale + 800w lossless PNG fallback <1MB each; masters never shipped); gateway full-bleed portal-centered-dark behind glass + LCP preload (imagesrcset proven in dist), landing portal-orbit-dark band object-position 72%, token-only scrims (color-mix over --canvas-night), contrast computed worst-case (gateway 7.33:1, landing 6.00:1 ink-hi; visual sign-off stays open per delivery rule 6); theme-gap documented (picture/preload key on prefers-color-scheme only; both surfaces pinned-night so dark masters correct; light companions generated-unwired); dist +10.24MB; 225-check spec incl. master hash re-verify. **AF-05** — server-authority validation: 800ms debounced poll keyed on real draft mutations (seq-token stale discard - honest AbortController equivalent documented), chips idle/loading/ready/unavailable (never silent offline), SAVE_SUCCESS/PUT-400/409 reuse server issues as latest result; Activate disabled on known issues with explanatory titles, confirm dialog lists localized issues; fixed toApiError dropping issues (1 line - was dead-codeing AF-04 banners); sync-guard AUTOMATED zero-dep scripts/check-graph-tokens.mjs (regex-extracts python codes/tokens vs TS arrays as ordered sequences + fa/en 130==130 parity) wired qa:tokens; client structural mirror REMOVED (no duplication). Combined gates: graph-consumer 93 + asset-delivery 225 PASS, admin check 0 + qa:tokens PASS, cms untouched this wave (no pytest delta), validator PASS. Commits: combined WF-08+WF-09 (both touched pages/index.astro) then AF-05.
- Why: Owner-mandated fastest parallel development.
- Scope / files: three agents' allowlists + this entry; shared-worktree concurrency held again.
- Commands or actions actually performed: combined verify -> 2 commits (combined web, admin) -> this entry.
- Verification actually performed and result: embedded; contrast + hash + sync-guard are machine-checked, visual crop sign-off remains owner.
- Decisions / assumptions: island deferral explicit (not silent partial); 800w PNG-only fallback (1600w would breach 1MB referenced-file rule); sharp-from-astro-tree avoids lockfile change; validation unavailable-state keeps 409 reachable (no lockout on hung poll).
- Deferred or risk IDs: G7 interactive 2D island (new explicit packet, after owner priority call vs launch); related-slugs read endpoint (BK-05 follow-up); light-theme art wiring (needs theme-following surface); gateway/landing visual crop sign-off (owner); remaining wave-10: WF-10 QA hardening (G9), WF-CLEAN sweep, AF-06 semantic previews, AF-07 i18n sweep.
- Doctrine compliance: WF-08 [7.1(3) props-in, list-first no-JS, 7.3 rows 1-8]; WF-09 [7.2(2) tokens-only scrims, 7.3(3) hex-grep clean, masters untampered machine-verified]; AF-05 [reducer-first additions, zero deps, i18n parity automated, deletion of duplicated logic = MODE 7.2].
## LOG-0278 — 2026-08-27 — Wave 8 parallel x4: route adoption 07E-07H all green — ALL EIGHT route families adopted

- Outcome: Seventh parallel wave (4 agents): **WF-07E** — writing index/series/tag -> EditorialIndexTemplate (featured/pagination/toc honestly unfilled - no loader flags/TOC data; spec asserts TableOfContents absence so future TOC forces conscious revisit), [slug] -> LongFormTemplate (SeriesNav + primary-tag siblings related; StoryBody preserved; reading time on detail lead only); Jalali PROOF in dist: fa '۱۹ اردیبهشت ۱۴۰۵' + '۶ دقیقه مطالعه' (Persian digits via formatNumber), en '24 August 2026'; RSS/blog redirects/JSON-LD/ogImage untouched (specs PASS); CMS-backed build 74p; 134 checks; WF-CLEAN rows: ArticleCard/ArticleDetail/TagList. **WF-07F** — teaching -> EditorialIndex + LongForm: real <ul> prerequisites/outcomes (sections OMITTED when absent - no 'None' fallbacks), MetadataGroup lead meta, raw-ISO last_updated -> formatDate Jalali, raw-hex unavailable box -> role=status tokens, unavailable-translation ContentState, zero LMS-invented markers asserted; level badge = raw CMS value (no dictionary exists - owner escalation like project_type); 52 checks; DEBT row: detail style block 42 lines vs 40 threshold (7.5.2-B, no natural seam, shared-file creation forbidden). **WF-07G** — about.astro + [locale]/about/{section}/{slug} + cv -> UtilityTemplate: About index WITHOUT breadcrumbs (IA S7), gated sub-routes WITH, radio-CSS tabs mechanism + ALL about-tabs selectors preserved verbatim (spec PASS x3 runs), sticky toolbar top: var(--space-sticky-offset) (bug-fix from reDesign_plan carried), journey Timeline from profile experience/education (omit-when-empty), gated-detail getStaticPaths logic preserved (cms-profile-build PASS incl. outage fail-build); route files ~950 lines each = documented 7.5.2-B exception (tabs mechanism inline; follow-up shared-tabs proposal suggested); legacy About.astro/Downloads.astro unimported (WF-CLEAN rows); fa diff: ZERO Arabic-block changes (byte-exact). **WF-07H** — contact -> UtilityTemplate + form rebuilt on ui/InputField/TextareaField/Button (frozen /api/contact field contract byte-for-byte; endpoint answered its own HTML status pages historically so page-side value loss was pre-existing); NEW retention: Astro.url.searchParams round-trip (?sent=1/0 + fields) feeding primitive value props (build-time renders empty - honest); success/error panels via ContentState ready/error (role=status/alert); privacy grep zero tel:/phone/gmail incl. dist re-check (old dead telHref helper removed); content.ts 8-line verifiable fix (sentHeading/errorHeading mirrored BYTE-EXACT from public_contact.py strings - codepoint-verified); ESCALATE: InputField lacks type/autocomplete/maxlength/rows seams (email degrades to text; server-side enforcement unchanged) -> WF-02-style mini-task proposed. Combined gates: 4 new adoption specs (134+52+85+51) + foundation 6 + about-tabs + cms-profile-build + health PASS; check 0 errors; build 58p snapshot atlas-free; validator PASS. Commits: 20c35b2/a235796/37c7f04/965c4b0.
- Why: Owner-mandated fastest parallel development; with this wave ALL EIGHT route families run on the six shared templates - the WF-05 contract is fully consumed.
- Scope / files: four agents' allowlists (07H content.ts 8-line verifiable fix reported per protocol) + this entry. Shared-worktree concurrency held (mutual untouched-diff reports; one transient dist race + one node crash re-run sequentially - no code implicated).
- Commands or actions actually performed: combined verify (4 new specs + about-tabs re-run + check + build + validator) -> 4 focused commits -> this entry.
- Verification actually performed and result: embedded; Jalali, disclosure, tabs-selectors, privacy greps all dist-level evidence.
- Decisions / assumptions: TOC absent-until-data-exists (spec-enforced absence); 'None'-fallback removals (honest omission); About breadcrumbs absent/sub-kept reading of IA S7; contact retention via query round-trip (documented live-UX consequence: CMS status pages remain the no-JS flow).
- Deferred or risk IDs: OWNER escalations now queued: project_type/role/methods/level label dictionaries (3 families render raw source values - no inventions); InputField seams mini-task; nav-capable lightbox variant; perspectives->researchFit mapping; More/ThemeToggle + composed-fa review lists (AF+WF). WF-CLEAN inventory grew: ArticleCard/ArticleDetail/TagList, About.astro/Downloads.astro, ResearchCatalog.astro, Landing legacy sections, CaseStudyDetail related-dup note. Remaining: WF-08 graph consumer, WF-09 asset delivery, WF-10 QA hardening, WF-CLEAN sweep, AF-05/06/07.
- Doctrine compliance: all four [7.1 layered composition, 7.2(1), 7.3 rows 1-3,5-8, adopt-equals-delete]; 07F+07G [7.5.2-B documented exceptions]; 07H [privacy sweep = G9 preview].
## LOG-0277 — 2026-08-27 — Wave 7 parallel x4: route adoption 07A-07D all green (launch-chain complete on Home)

- Outcome: Sixth parallel wave (4 agents, disjoint route families): **WF-07A (LAUNCH-CRITICAL)** — Home rebuilt onto HomeTemplate with CMS composition: homeComposition.ts never-throw adapter (200->mapped order; 404/transport/malformed->null+warn-once gated by isCmsOriginBuild; malformed-published also falls back so HomeTemplate never throws), DEFAULT_HOME_ORDER canonical 8 in site.ts (+HomeBlock type), blocks wired to REAL loaders (projects/publications/writing previews render when non-empty, honest-omit otherwise; graph omitted->WF-08; researchFit/journey omitted - no existing data seam, ESCALATE to owner whether perspectives grid maps to researchFit); Landing.astro kept as zero-import night-stage token-remapper (ADR-0031 canvas readability in both themes); 4 unimported legacy sections flagged for LEGACY-INVENTORY (PerspectiveGrid/EvidenceSection/WritingLatest/ContactCTA); A1-stack build with empty composition exit 0 + warn path proven; home-cms-build + cms-profile-build + mobile-overflow (CI parity) PASS. **WF-07B** — research index/topics -> CollectionIndex (ContentRow results, Chip taxonomy nav, URL filters preserved), statement -> LongForm (PDF link preserved), publications index (PublicationRow, canonical anchor-wrap -> p8 parity exact) + detail -> EvidenceDetail (MetadataGroup, bdi DOI, citation preserved); honest-empty ContentState everywhere; ResearchCatalog.astro now dead (WF-CLEAN row flagged); 100-check spec; 57p build. **WF-07C** — projects onto templates: GET-form no-JS filters (params + bindCatalog behavior preserved), data-project-catalog CI marker kept, sanitized-evidence disclosure proven in rendered dist, depth Badge, unavailable-translation ContentState; 45-check spec; ESCALATE: project_type/role/methods_summary dictionary labels do not exist anywhere (raw source value rendered today; owner fa/en labels needed - nothing invented). **WF-07D** — creative onto templates: MediaTile grid (honest failed-variant placeholders until CMS list grows image fields), shared Wave-2 F7 lightbox adopted (bespoke dialog deleted; prev/next/arrow-nav retired with it - ESCALATE nav-capable lightbox variant), access_state honesty (restricted: no dialog/no asset/no gallery, verified in dist), rights/license metadata; 64-check spec. Combined verify: 4 new specs + all 6 foundation specs + p8 parity PASS; check 0 errors; build 58p atlas-free; validator PASS. Commits: 07A->07B->07C->07D focused.
- Why: Owner-mandated fastest parallel development; Home launch-critical chain (BK-01->AB-02->AF-01->WF-07A) is now code-complete pending AF-01..04 owner review on the SPA side.
- Scope / files: four agents' allowlists + this entry. Shared-worktree concurrency held (each agent reported foreign diffs untouched).
- Commands or actions actually performed: combined verify (4 new + 6 foundation + p8 specs, check, build, validator) -> 4 focused commits -> this entry.
- Verification actually performed and result: embedded; p8 parity explicitly re-proven post-adoption; A1-composition fallback path proven by 07A on the live local stack.
- Decisions / assumptions: adapter allow-404 design (documented LAUNCH-CRITICAL contract); Landing retained as night-stage wrapper rather than deleted (deleting would break hero/CTA night readability - cleaner than forced retirement); statement page gains family contact-note tail (IA S6); PublicationRow anchor-wrap for canonical links.
- Deferred or risk IDs: OWNER DECISIONS queued: (1) perspectives-grid -> researchFit mapping; (2) project_type/role/methods labels fa+en; (3) nav-capable lightbox variant; (4) More/ThemeToggle + all composed-fa strings review. WF-CLEAN rows: ResearchCatalog.astro, Landing legacy sections, CaseStudyDetail related-duplication note. Remaining packets: WF-07E..H (next wave), WF-08 graph consumer, WF-09 assets, WF-10 QA, AF-05/06/07.
- Doctrine compliance: all four [7.1 layered composition, 7.2(1) new-family=template+adapter+spec only, 7.3 rows 1-3,5-8, adopt-equals-delete applied/flagged]; 07A additionally [7.5 completeness-over-budget n/a, honest-omit IA rule].
## LOG-0276 — 2026-08-27 — Wave 6 parallel x3: WF-06 Atlas + AF-04 Graph Editor + AB-07 consolidation (636 tests)

- Outcome: Fifth parallel wave (3 agents): **WF-06** — local-only Visual Atlas: Astro 7.2.4 mechanism = injectRoute inside an astro:config:setup inline integration gated on DESIGN_ATLAS==='1'; scripts/design-atlas.mjs cross-platform launcher (--build/--dev, Windows-safe); src/design-atlas/** (standalone nav-free shell - documented deviation from BaseLayout which would drag nav/CMS coupling; UNPUBLISHED banner; theme/dir/viewport/reduced-motion controls scoped to specimen; 42 data-atlas-id hooks; real FA strings via content.ts for ASCII-file constraint); TDD dual-build gate: default build 58p with 0 _design paths/strings/hooks (G4), DESIGN_ATLAS=1 emits 59th page /_design/index.html (G5), pagefind skips it; qa/design-atlas.spec.mjs re-runs BOTH builds itself + source guards; all 5 prior web specs still PASS. **AF-04** — Graph Editor shell: pure canvas.ts viewport math (cursor-centered clamped zoom 0.25-3 with effective-factor trick, DPR pixel-snap, world<->screen), 22-action reducer (saved-vs-draft isolation, dirty-by-content payloadEquals), SVG canvas (arrow markers, edge hit-areas, role=img + sr-only fallback cap 200), InspectorPanel (node+edge forms, relatedRecords add/dedupe), VersionsPanel (status pills fa byte-exact incl ZWNJ hex-verified, groups render-only v1), Save(PUT 400-issues banner)/Activate(confirm + VALIDATION_BLOCKED banner, disabled while dirty), route+nav+64/64 i18n parity; api.ts additive issues passthrough (frozen client otherwise drops 400 bodies); tsc 0, build green, RTL grep 0. **AB-07** — pure consolidation +357/-357: 20+ duplicated tokens + If-Match helpers (home's empty-locale If-Match:'"" semantics preserved via parameterized _require_if_match) + audit writer + GRAPH_RELATED_FAMILIES graduated into admin_common; graph's _format_iso byte-identical rename unified; GRAPH_ISSUE_CODES stays in the pure validator (doctrine: no Django imports); 1 new cross-check test (wire table == hardcoded); OpenAPI verified staff-gated: 47 schema paths, 15 AB paths/16 ops zero-missing; 636 passed; BOM incident (transient scripted replace) detected+removed byte-level. Combined gates: atlas spec PASS, cms 636, admin tsc 0, validator PASS. Commits: 72d6582/eb7f5bd/fb0b2a3(+test fixup).
- Why: Owner-mandated fastest parallel development; this closes ALL admin-side redesign packets (AB-00..07) and the web foundation (WF-00..06) - remaining: route adoption 07A..H, graph consumer 08, assets 09, QA 10, AF-05..07.
- Scope / files: three agents' allowlists + one missed test fixup + this entry.
- Commands or actions actually performed: combined verify (atlas spec re-running dual builds, cms pytest, admin tsc, validator) -> 3 commits + test fixup -> this entry.
- Verification actually performed and result: embedded; agents' zone purity held (AB-07 explicitly quarantined concurrent AF/WF diffs in its stat audit).
- Decisions / assumptions: atlas standalone shell (no BaseLayout) is a documented G4-scope deviation; multiple h1s allowed INSIDE the dev-only atlas (template anatomy preserved); AF-04 iconRole stored-not-drawn v1; edge keyboard-delete deferred; Activate disabled while dirty (server validates stored payload).
- Deferred or risk IDs: AF-04 composed-fa owner-review list (40+ keys) appended to the carried list; arrowhead under target rect at close range (cosmetic v1); AF-05 (validation polling UX) + AF-06 (semantic previews) + AF-07 (i18n sweep) remain; WF-07A..H next wave.
- Doctrine compliance: WF-06 [7.1 layering (imports real components, never copies), G4 isolation]; AF-04 [reducer-first, zero deps, RTL logical-only, state isolation]; AB-07 [AB doctrine rule 1 (admin_common single home), gates 1-4 incl. zone-purity stat audit].
## LOG-0275 — 2026-08-27 — Wave 5 parallel x4: BK-05 + AB-05/06 + WF-05 + AF-03 all green (635 tests)

- Outcome: Fourth parallel wave (4 agents): **BK-05** — GET /api/graph/{locale} public read: active GraphVersion only, camelContract exact (key-omission for blank summary/colorRole/iconRole/explanation, position omitted unless x+y pinned, z-optional), stable content-derived edge id {source}->{target}:{relationType}, relatedRecords fail-closed re-check at read (batched per ContentType, no N+1; draft/dangling/no-gate omitted), groups excluded v1; 404 fail-closed; view 9 lines + module service (doctrine 8.1); 12 tests; **569 standalone**. **AB-05+AB-06** — pure dmin_graph_validate.py (injected resolver, zero ORM at top; GRAPH_ISSUE_CODES: DUPLICATE_NODE_ID/SELF_EDGE/DUPLICATE_EDGE[+swapped-undirected]/BROKEN_RELATED/MISSING_ACCESSIBLE_LABEL/BAD_WEIGHT/MISSING_POSITION with graph.* messageTokens) + dmin_graph.py 5 endpoints/6 ops (versions CRUD, payload PUT validate-first replace-all in tx, activate archives previous + VALIDATION_BLOCKED, validation report); integration strictness: BROKEN_RELATED at PUT (model clean tolerates post-hoc deletion); 4 structural 400 guards (UNKNOWN_EDGE_ENDPOINT/DUPLICATE_RELATED/UNKNOWN_GROUP_MEMBER/DUPLICATE_GROUP_MEMBER) prevent 500s; BAD_WEIGHT integral-only (storage PositiveSmallInt - fractional request filed to BK-REQUESTS); 66 tests; **635 combined**. **WF-05** — six templates as pure slot composition (zero imports, shell owns main landmark - documented deviation), Home guarded 8-block order prop (canonical default = homepageOrder, unknown/dup frames throw), detail templates throw on empty lead/body (no empty shells), optional regions Astro.slots.has-guarded (no wrapper gaps); ROUTE-MAP asserted exactly = IA live families; 272-check spec; check 0 errors, 58p build. **AF-03** — media presentation UI landed in MediaEditDrawer (survey overruled MediaPicker placement: onSelect-closes contract leaks edit UX into 6 flows): MediaFocalPicker (pointer-capture + pure math exports + role=slider axes + RTL-agnostic SVG), rights fa/en, module-cached licence select + clear, captions (max 300), session-touched-only PATCH with If-Match via updatedAt, 409 -> generalized RevisionConflictDialog, token messages localized; 22 i18n keys parity 70/70; physical left/right grep 0; deps frozen. Combined gates: web specs (272+240+98+194+token) PASS, check 0 errors, cms **635 passed** + ruff clean, admin build ok, validator PASS. Commits: d43159e / 9679189 / 8cff464 / 0e8c193 + contracts addendum in BK-05 commit.
- Why: Owner-mandated fastest parallel development; launch chain BK-01->AB-02->AF-01->WF-07A now has ALL admin/graph prerequisites green.
- Scope / files: four agents' allowlists + contracts addendum + this entry. AB-06 duplicated the related-family mapping with SYNC-GUARD comments (BK-05 landed mid-flight without exported helper) - graduation to a shared helper scheduled AB-07.
- Commands or actions actually performed: combined verify (5 web specs + check, full cms pytest+ruff, admin build, validator) -> addendum -> 4 focused commits -> this entry.
- Verification actually performed and result: embedded; BK-05/AB-06 edge-id composers independently converged byte-identical (cross-validated parity).
- Decisions / assumptions: envelope {nodes,edges} without revision (not in §4 contract - flagged to WF-08); multiple drafts per locale allowed (DB constrains active only); AB-06 validator strict-at-PUT (all codes blocking) while BK-05 read re-checks related fail-closed - two independent honesty layers; AF-03 no read endpoint exists (AB-04 froze none) so section starts unknown-state, PATCH-only-session-touched (documented UX consequence).
- Deferred or risk IDs: BK-REQUESTS: fractional graph weights (needs storage widen); AB-07: graduate 8 new stable tokens + shared family-mapping helper; AF composed-fa owner-review list grew (media strings); dictionary keys More/ThemeToggle still carried; AF test-runner decision still open (pure seams exported).
- Doctrine compliance: BK-05 [8.1.1 view 9-line + service, 8.3.1-4 (contracts addendum same-commit), 8.2.4 batching]; AB-05/06 [AB layer 1-4 + versioned-namespace readiness, validator-as-library pattern, contract-first sync-guards, every code tested]; WF-05 [7.1(1,4,5), 7.2(1) - new family = one template, 7.3 rows 1-3,5,7]; AF-03 [component reuse kit classes, zero deps, RTL logical-only, i18n complete-at-commit].
## LOG-0274 — 2026-08-27 — Wave 4 recovered after tool-abort: TOK-DANGER + BK-04 + AB-04 + WF-04 + AF-02 (557 tests)

- Outcome: Wave 4 dispatched 5 parallel agents; 4 task channels aborted mid-run (tool-level, after file writes, before final report). Recovery: verified each packet's on-disk state, fixed the残 partials by hand, gated everything green, committed as 6 focused commits. **TOK-DANGER (completed by its agent incl. LOG-0273):** semantic danger pair Light #b3261e (6.13/6.54:1) + Dark #ef6b68 (6.21:1), night --danger aliased to it (retires WF-02's 2.79:1 escalate with ZERO component edits); tokens.json 1.0.0 -> 1.1.0 (additive danger both maps); spec gates >=3:1 x3. **BK-04** — GraphVersion (UniqueConstraint one-active-per-locale via condition), GraphNode (unique version+node_id), GraphEdge (clean(): same-version endpoints + swapped-undirected dup rule -> ValidationError; DB deliberately unconstrained per doctrine 8.2), GraphGroup (JSON node_ids v1); content.0019 PG-applied; follow-up: public_media_ref gains focal {x,y} only when set. Tests: 33 graph (rewrote 1 aborted-test to the model's real clean() contract after my splice briefly broke syntax - fixed) + BK-03 file test_media_presentation_metadata.py now tracked (was missed from bf1d3aa). **AB-04** — admin_media_ext.py: PATCH /admin/media/{id}/presentation (subset-only, explicit-null clears, UNKNOWN_FIELD/OUT_OF_RANGE/UNKNOWN_LICENSE tokens, If-Match 428/409, audit) + GET /admin/media/licenses; 23 tests incl. public-payload guard updated for focal-in-payload (rights/licence/captions proven absent). **WF-04** — 10 content components (SectionLead/FeaturedRecord/ContentRow/PublicationRow/MetadataGroup/Timeline/TimelineNode/MediaTile/TableOfContents/ContactCTA) composing ui primitives; 240-check QA gate; 2 spec markers realigned to the stricter implemented patterns (visibleTags.length>0 &&, visibleAuthors.join after filter). **AF-02** — Timeline Editor: useOrderedDraftList pure helpers, TimelineRow/CreateRowDialog/ConfirmDeleteDialog, reducer with 409/428/delete flow (2 TS fixes: select-union casts + missing DELETE_START/DELETE_ERROR cases in exhaustive switch), RevisionConflictDialog generalized, AdminDialog extracted; tsc 0 errors, vite build green, deps frozen. Combined gates at close: web 4 specs PASS (98+194+240+token), cms **557 passed** + ruff clean, admin build ok, validator PASS.
- Why: Owner continued after abort; fastest path = salvage verified on-disk work rather than re-dispatch blind.
- Scope / files: the five packets' allowlists + WORK_LOG (agent-authored LOG-0273 kept) + this entry. Commit chain: b1a1ae2 (BK-03 test fixup) -> 485b47b (BK-04) -> 115e5ed (AB-04) -> 5318eab (WF-04) -> 3f5098e (AF-02) -> ea04598 (TOK-DANGER).
- Commands or actions actually performed: per-file state audit after abort -> targeted gates per packet -> hand-fixes (spec markers x2, graph test rewrite + syntax repair, AB-04 public assertion update, AF TS union casts + reducer cases, E501 wraps) -> full combined verify -> focused commits -> this entry.
- Verification actually performed and result: all gates green as listed; no agent report claimed unverified PASS - every number above re-ran by lead on the combined tree.
- Decisions / assumptions: swapped-undirected dup stays clean()-level (doctrine: no DB over-constraint; validator layer AB-06 will re-check on payload save); public focal added as nested {x,y} omitted-when-unset (honest-absent convention); AF DELETE_START = no-op state (no deletingRowIds set exists; per-row busy UX deferred), DELETE_ERROR -> toast delete-failed + candidate dialog dismissed.
- Deferred or risk IDs: AF-03 (media picker focal/rights UI) now unblocked by AB-04 -> wave-5; AB-06 validator + BK-05 public graph read unblocked by BK-04 -> wave-5; AF graph editor (AF-04) after AB-05/06; dictionary keys More/ThemeToggle + AF composed-fa owner review still carried.
- Doctrine compliance: TOK [7.2(2) token-first, 7.3(3)]; BK-04 [8.1.2, 8.2.1, 8.2.3, 8.3.1-3]; AB-04 [AB layer rules 1-4 + gates 1-4]; WF-04 [7.1(1-5) composition over primitives, 7.3 rows 1-3,5-7]; AF-02 [reducer-first, i18n byte-exact additions, zero deps, RTL logical-only].
## LOG-0273 — 2026-08-27 — TOK-DANGER wave-4 micro packet: semantic danger pair landed + gated (WF-02 escalation CLOSED)

- Outcome: Both-theme-valid semantic danger token shipped and enforced (closes the danger-token escalation carried in LOG-0271/LOG-0272 deferred items). Computed with the spec's own WCAG luminance/contrastRatio helpers over the packet's candidates: Light #b3261e / #a8211b / #9c1f18 all passed >=3:1 on canvas #f7f8f5 AND surface #ffffff (6.13+6.54 / 6.80+7.25 / 7.51+8.01) - chose **#b3261e** (lightest = closest to brand hue, >=6:1 margin). Dark #ef6b68 / #f2a49c / #e5857d all passed >=3:1 on canvas #071225 (6.21 / 9.41 / 7.11) - chose **#ef6b68** (nearest hue to old ad-hoc #ff6b5e, minimal live-UI shift; 5.95:1 on dark surface #0b1630, recorded report-only). Landing: global.css gains --color-danger in BOTH the Light @theme table and the [data-theme="dark"] block; tokens.json semanticLight/semanticDark gain the `danger` role (version 1.0.0 -> 1.1.0, additive); qa/design-tokens.spec.mjs ROLE_TO_PROP gains danger -> --color-danger and 3 new >=3:1 gates (Light danger/canvas, Light danger/surface, Dark danger/canvas); DESIGN-CONTRACT.md gets §2.1/§2.2 danger rows + §0 version pin update. The escalated 2.79:1 failure is fixed at consumer level with ZERO component edits: night token --danger now aliases var(--color-danger), so the landed InputField/TextareaField error states resolve the theme-valid value in both themes (they previously consumed ad-hoc #ff6b5e = 2.62:1 light canvas / 2.79:1 white). Parallel-agent content-components zone untouched; no git actions.
- Why: WF-02 escalated that components need a semantic danger color valid in BOTH themes (non-text >=3:1 on each canvas); wave-4 TOK micro packet was the scheduled owner.
- Scope / files: apps/web/src/styles/global.css, Assets/site-redesign/implementation-reference/agent-kit/tokens.json, apps/web/qa/design-tokens.spec.mjs, docs/contracts/DESIGN-CONTRACT.md, this entry.
- Commands or actions actually performed: candidate ratio computation (temp node script reusing the spec's math verbatim) -> baseline qa/design-tokens.spec.mjs run (PASS, 20/20 roles) -> surgical edits -> spec rerun + agent-kit validate.mjs rerun.
- Verification actually performed and result: design-tokens.spec: PASS - 21 Light roles byte-for-role, 1 [data-theme="dark"] block with 21 Dark roles, no raw hex outside token areas, contrast gates met (both themes); danger gates 6.13 / 6.54 / 6.21 all PASS (>= 3:1). agent-kit validate.mjs: PASS (24 components, 6 templates, 10 asset references, 14 reference files).
- Decisions / assumptions: --danger alias chosen over editing InputField/TextareaField (token-only fix keeps component files untouched and out of the parallel agent's blast radius; always-night gateway resolves the dark value, light pages get #b3261e); tokens.json minor version bump records the additive contract change and the DESIGN-CONTRACT §0 pin was updated to match; Dark danger/surface left report-only (task gate scope = canvases) at 5.95:1.
- Deferred or risk IDs: none opened - the WF-02 carried escalation (LOG-0271 item 1, LOG-0272 item 1) is CLOSED here.
- Doctrine compliance: token addition done per DESIGN-CONTRACT §2 rule ("add it to global.css + tokens.json in the same ATLAS-01 task and say so in the Work Log") with contract-table parity in the same packet; gate extension shipped same-commit per the WF-01 FAIL-first/byte-for-role precedent; no new dependencies; G1 byte-for-role holds in both themes.

## LOG-0272 — 2026-08-26 — Wave 3 parallel x4: BK-03 + AB-03 + WF-03 + AF-00/AF-01 all green (455->497 tests)

- Outcome: Third parallel wave (4 agents, disjoint zones): **BK-03** — MediaLicense model + Media additive caption_fa/en, focal_x/y (Decimal 5,2, nullable-safe CheckConstraints 0-100), rights_statement_{fa,en}, license FK SET_NULL; migration media.0003 PG-applied; 13 tests incl. exact-key-set proof that rights/license/caption stay OUT of the public payload ({url,alt,mime,title,size} unchanged; public focal parity deferred as tiny BK follow-up on public_urls.py); 455 passed standalone. **AB-03** — admin_timeline.py: GET(list+profile filter)/POST(append or after_id with +1 shift)/PATCH/DELETE(reorder registered BEFORE {id} route — route-order bug caught by tests)/reorder permutation; If-Match 428/409 parity; tokens BAD_TYPE/INVALID_DETAIL_URL/BAD_WEIGHT/UNKNOWN_ID/DUPLICATE_ORDER; audit timeline.*; 42 tests; full 497. **WF-03** — shell rebuilt on primitives: BaseLayout pre-paint theme initializer (localStorage theme | prefers, default system per owner), Header (logo /logo.png 256x233, parent-section activation, More details no-JS with aria-current, ThemeToggle via IconButton, honest alternateHref switch, glass), Footer full live sitemap, Breadcrumbs aria landmark, gateway standalone always-night with ui/Button pair; NEW 98-check public-shell spec; real mobile-overflow run PASS (6 URLs x 4 widths); 58 pages, dist/_design absent. **AF-00/AF-01** — survey (registry entities.ts, If-Match precedent api.ts, NO i18n existed -> created redesign.fa/en + tRedesign, NAV_GROUPS, banner-not-toast kit pattern, vite proxy fixed to :18000); adminApiExt.ts typed client; pure reducer (15 actions, canonical-row fill, swap+renumber); HomeComposerPage (locale tabs, 8 rows, a11y arrows, Save/Validate, revision chip, beforeunload); RevisionConflictDialog (focus trap + Esc + restore, reuses admin-modal classes); tsc 0 errors, vite build green, deps frozen, physical left/right grep = 0. Combined-tree verify: 2 web specs + 98-check PASS, cms 497 passed, admin build ok, validator PASS. Commits: bf1d3aa/73ac36b/9fcc6a3/6a4ac0b.
- Why: Owner-mandated fastest parallel development.
- Scope / files: four agents' allowlists only + this entry.
- Commands or actions actually performed: combined verification -> 4 focused commits -> this entry.
- Verification actually performed and result: embedded; agent zone-purity held again (each reported foreign in-flight diffs as untouched).
- Decisions / assumptions: gateway pinned data-theme=dark (always-night, status quo) outside toggle reach; GET timeline returns bare array (spec-exact) over TRACK envelope wording; POST create returns 200 per repo convention; AF-01 static build is the gate until owner runs A1 stack.
- Deferred or risk IDs: (1) danger-token pair escalate carried from WF-02 -> wave-4 TOK micro packet; (2) dictionary gaps 'More'/'Toggle theme' in content.ts (WF-03 hardcode precedent kept; owner keys later); (3) AF composed fa strings need owner review incl. semantic mismatch of 'بارگذاری مجدد تغییرات من' as reload-server action; (4) public focal parity follow-up folded into BK-04 dispatch; (5) AF has no test runner (deps frozen) — reducer pure, runner adoption deferred.
- Doctrine compliance: BK-03 [8.2.1, 8.2.3 indexes/constraints same-commit, 8.3.1-3]; AB-03 [layer rules 1-4, gates 1-4 (route-order regression caught, token registry tested, zone purity 3 files)]; WF-03 [7.1 layering via primitives, 7.3 rows 1,2,3,5,6,8 (mobile-overflow real run),10]; AF-01 [component-driven layers, reducer-first, i18n complete-at-commit byte-exact, zero deps, RTL logical-only, deletion-friendly (reused kit modal classes)].
## LOG-0271 — 2026-08-26 — Wave 2 parallel: BK-02 + AB-00/AB-02 + WF-02 all green (419->442 tests)

- Outcome: Second parallel wave (3 agents, disjoint zones, no-git rule): **BK-02** — TimelineRecord (locale, nullable FK->Profile SET_NULL, type choices experience/education/project/milestone/talk/publication, label/period_label/body/role/weight/order>=1 CheckConstraint, detail_url validator rejecting javascript:/ftp:///'//host', LifecycleMixin) + content.0018_timeline_records (PG-applied) + or_locale/published_for_locale helpers + 17 tests; **419 passed** standalone. **AB-00+AB-02** — survey: single NinjaAPI dmin_api mounted at config/urls.py:25 (no admin_urls.py needed — routers append via add_router; guarded # [ADMIN-API] Track AB marker used); auth _require_admin_otp+_check_csrf reused verbatim; AuditLog writer mirrored. Shipped dmin_home.py: GET/PUT/validate on /api/v1/admin/home-modules/{locale} per frozen contract — PUT validate-first replace-all in tx (upsert/delete), If-Match ms-precision (428 PRECONDITION_REQUIRED new stable token; 409 STALE_REVISION per newer AB contract, documented divergence from older CONFLICT style), +2 agent-added tokens DUPLICATE_KEY/TOO_LONG (model-constrained cases; graduation to admin_common deferred to AB-07); 23 tests; PG shell smoke GET/PUT/GET/404/401 live. **WF-02** — 8 primitives (Button/IconButton/LinkAction/Chip/Badge/InputField/TextareaField/ContentState) with native semantics, 44px floors, aria wiring, hover/focus twins, tokens-only scoped styles, RTL arrow flip, 6-kind ContentState incl. role=alert/aria-busy; global.css ZERO edits (scoped styles, allowance unused); 194-check structural QA gate; npm check 0 errors, build 58p, dist/_design absent. Combined-tree integration verify: both web specs PASS, cms **442 passed** (402+17+23), ruff clean, validator PASS. Commits: BK-02 -> AB-02 -> WF-02 (focused), then this entry.
- Why: Owner-mandated fastest parallel development.
- Scope / files: exactly the three agents' allowlists + this entry. Escalation carried forward: tokens.json lacks a both-theme-valid semantic danger pair (WF-02 agent: #ff6b5e on Light = 2.79:1 non-text; current components carry meaning via icon+text+aria-invalid so not color-only, but a WF-01-style micro token task is owed) — scheduled next to global.css owner (wave-4 micro) to avoid concurrent same-file edits.
- Commands or actions actually performed: combined verify (2 web specs + full cms pytest + ruff + validator) -> 3 focused commits -> this entry.
- Verification actually performed and result: embedded tails; zone purity confirmed by agents (WF-02 saw cms diffs as 'another track', AB saw timeline diffs, BK-02 saw admin diffs — mutual exclusivity held).
- Decisions / assumptions: If-Match missing-header split (428) accepted as contract-frozen newer vocabulary (documented divergence from admin_content CONFLICT conflation); TimelineRecord attach=SET_NULL so records survive profile deletion; order default=1 to satisfy CheckConstraint on bare create().
- Deferred or risk IDs: ESCALATE carried: danger-token pair (see above) — to be closed in wave-4 micro packet.
- Doctrine compliance: BK-02 [8.1.2 one-concept-one-model additive, 8.2.1 descriptive migration, 8.3.1-3]; AB-02 [layer rules 1-4: one-domain-module admin_home.py, additive-only registration marker, HTTP-thin via pure validator fn, uniform auth+audit; gates 1-4 (zone purity 3-file diff, auth parity copied, token registry tested)]; WF-02 [7.1(1-5), 7.3 rows 1,2,3,5,6,7,10] (component purity: no locale copy, no fetch, tokens-only).
## LOG-0270 — 2026-08-26 — Wave 1 parallel execution: BK-01 + WF-01 + BK-00 all green (3 agents, disjoint zones)

- Outcome: First parallel build wave landed on baseline 4f96843 via three concurrent agents (same-tree, disjoint ownership per MODE §8; agents forbidden to git-commit; integration lead committed sequentially): **BK-01** — HomeModule per-locale rows (locale/key/visible/order/selection_mode/provenance_note, UniqueConstraint(locale,key), LifecycleMixin) + migration content.0017_home_modules + public GET /api/home-composition/{locale} (200 {revision: ISO, modules:[{key,order}]} published+visible only; 404 fail-closed incl. invalid locale; interpretation note: per-locale-row reading chosen over contradictory visible_{fa,en}+unique(locale,key)) + 9 tests; **402 passed** (393+9); migration applied on real Postgres A1 stack; ruff clean. **WF-01** — TDD qa/design-tokens.spec.mjs (FAIL-first 25 problems -> PASS): 20 Light roles byte-for-role, single [data-theme="dark"] block with 20 semanticDark roles, hex-containment via new design-tokens markers (2 pre-existing stray hexes tokenized to ar(--canvas-night), value-identical), WCAG gates both themes (Light 15.04/4.94/3.60; Dark 16.90/10.60/4.85; brand pairs printed 4.76-9.92); src/design-system/contracts.ts with exactly ThemeName/Direction/ContentState; DESIGN-CONTRACT §2.2 now 'runtime-ready behind [data-theme], default theme = system (owner 2026-08-26), ThemeToggle = WF-03'; 
pm run check 0 errors, build 58 pages with dist/_design ABSENT, validator PASS. **BK-00** — docs/plan/BK-DATA-CONTRACTS.md: 32 entities, 37 public GETs, 12 gap rows; top findings: profiles aggregate is camelCase Django views (snake_case ProfileOut shadowed), media metadata thinnest (BK-03 purely additive), graph fully absent (BK-04/05), Collection model stranded (candidate for home previews), BK-06 likely near-empty (talk slides/course outcomes exist) while BK-02 timeline is the honest gap. Addendum row appended for the shipped BK-01 endpoint. Integration verify on the combined tree: token spec PASS, pytest 402, validator PASS. Commits: 964eb45 (BK-00), 5abbfdb (BK-01), 1baa5ec (WF-01).
- Why: Owner ordered fastest-possible development start with parallel agents.
- Scope / files: exactly the two agents' allowlists + the new contracts doc + this entry. Wave 1 commits keep WORK_LOG outside packet commits (ledger written by lead, per anti-race protocol).
- Commands or actions actually performed: parallel task dispatch x3 (disjoint allowlists, no-git rule, encoding rule from LOG-0269 lesson) -> combined-tree verification (token spec, full cms pytest, validator) -> addendum -> 3 focused commits -> this entry.
- Verification actually performed and result: see embedded tails; nothing committed without green gates; Light tokens untouched (spec-asserted byte-for-role).
- Decisions / assumptions: revision = ISO timestamp (agent choice, documented, cache-friendly); stricter 404 on invalid locale than sibling list endpoints (per row contract); 	okens.json semanticDark.status stays 'design-target' pending owner/ATLAS-12 flip (DESIGN-CONTRACT is runtime contract owner; reference pack read-only per AGENTS); vitest was installed as a transient dependency-resolve during agent's documented npm install step (package.json/lockfile hashes verified unchanged).
- Deferred or risk IDs: none opened. AB-02/AF-01/WF-07A launch chain: BK-01 side now DONE -> next wave dispatches WF-02 (primitives, dep WF-01 done), AB-00+AB-02 (admin endpoints, dep BK-01 done), BK-02 (timeline records, dep BK-00 done) — three disjoint zones again.
- Doctrine compliance: BK-01 [§8.1.1 manager/service gate, §8.2.1/8.2.3, §8.3.1-4, §8.3.6] (per agent report; BK-DATA-CONTRACTS addendum closes §8.3.4); WF-01 [§7.1(2,3), §7.2(2), §7.3 rows 1,2,3,10] (FAIL-first evidence, token-only values, TDD same-commit, no new deps); BK-00 [§8.3.4 doc-parity foundation] (doc-first sheet enabling other tracks).
## LOG-0269 — 2026-08-26 — Owner decision: Home v1 = Option B (CMS-complete); encoding-incident repair across 15 docs

- Outcome: **Home v1 composition = Option B (CMS-complete)** — launch gate is the chain BK-01 (model + public read GET /api/home-composition/{locale}, 404 fail-closed when nothing published) -> AB-02 (admin bulk PUT + validate) -> AF-01 (Home Composer) -> WF-07A (CMS-driven consumption with documented DEFAULT_HOME_ORDER fallback only when 404; that 404 must not fail builds). BK-01 and WF-07A rows updated to LAUNCH-CRITICAL with the contract; plan-index sequencing line records the decision. **Incident:** several PowerShell edit passes used Get-Content -Raw (ANSI in PS5.1) + Set-Content -Encoding UTF8, mojibaking every non-ASCII byte in 15 touched docs (em-dashes and, in docs/status/WORK_LOG.md, the historical Persian entries were further destroyed to U+FFFD/'?' lossy). Repairs: single-layer mojibake in 15 files restored via cp1252->UTF8 round-trip (0 residual); WORK_LOG Persian history reconstructed by concatenating our 6 English entries (0263-0268) with origin's clean blob 75e1f07:docs/status/WORK_LOG.md (Persian probes pass); 9 self-inflicted U+FFFD spots in manifest/plan/tracks restored to em-dashes; plan-index Option-B line rewritten cleanly. Validator PASS post-repair.
- Why: Owner selected Option B verbatim from the Home-v1 decision brief.
- Scope / files: docs/plan/TRACK-BK-backend-cms-task-list.md (BK-01 row), docs/plan/TRACK-WF-public-frontend-task-list.md (WF-07A row), docs/plan/README.md (decision line), plus encoding repair across AGENTS.md, PROJECT_MANIFEST.md, README.md, Task-list.md, docs/README.md, ATLAS-frontend-rebuild-task-spec.md, all 5 TRACK-*.md, BACKLOG.md, deferred-validation.md, WORK_LOG.md. No runtime/source change.
- Commands or actions actually performed: detection regex over active md set (15 hits) -> .NET byte-safe round-trip repair loop -> Persian probe failure on WORK_LOG -> git cat-file blob 75e1f07:... via cmd redirect (byte-faithful) -> splice rebuild -> FFFD context inspection (8 own-damage vs legacy-loss triage) -> contextual em-dash restoration -> validator PASS -> commit 8184f16 -> this entry.
- Verification actually performed and result: residual U+FFFD = 0 in manifest/plan/track docs; Persian probes (پروداکشن/مالک-era strings) TRUE in rebuilt WORK_LOG; BK-01 line 44 + WF-07A line 61 carry LAUNCH-CRITICAL contracts; validator PASS.
- Decisions / assumptions: WORK_LOG entries 0246-0253 from our superseded line remain ONLY on ackup/pre-sync-2026-08-26 (their numbers collide with origin entries of same ID but different content — documented repo precedent). Task-list.md Persian sections were ALREADY '?'-destroyed before this session (verified: origin/main copy equally damaged); reconstruction deferred — reopening as a backlog-style archaeology ticket is left to the owner (old clean version would need deep-history mining; the file is a historical ledger superseded by docs/plan/README.md §1b for execution). BACKLOG.md/deferred-validation.md retained their pre-session U+FFFD spots (pre-existing loss, not ours); flagged here rather than silently kept ambiguous.
- Deferred or risk IDs: none formally opened per protocol limits of this pass; the two legacy-loss flags above are recorded here as the durable record.
- Rollback / recovery: encoding state is now STRICTLY BETTER than pre-repair for all 15 files; git history retains both damaged (6222ef1^) and repaired (8184f16) states. Option-B doc rows revert by reverting 8184f16 + this entry's rows if the owner ever flips to Option A.
## LOG-0268 � 2026-08-26 � Sync main with origin (ADR-0032), WF-00 baseline freeze, owner decisions, A1 local stack live

- Outcome: (1) **Baseline reconciled and pushed.** Local P14 line (6 commits) + our 3 grouped commits merged with origin/main's 34 (Waves A�C, ADR-0031/0032): merge 700badb, reconcile 6455050, sweep 4ece1de, logo pack 1d643ef ? pushed 75e1f07..1d643ef. **Material discovery reconciled:** admin SPA is the independent pps/admin project (ADR-0032; pps/cms/admin-frontend no longer exists on origin; S5 cutover 2026-08-25 LOG-0251; compose target now db+cms+web+admin+caddy; live Caddy = infra/caddy/Caddyfile.compose). All four track docs, governance and tooling swept: dmin-frontend ? pps/admin (active docs 0 remaining; Task-list historical audit lines kept verbatim). Origin WORK_LOG (max 0256) unioned with our 0263�0267 (our older 0246�0253 remain on ackup/pre-sync-2026-08-26; duplicate-ID gap 0257�0262 left unassigned). (2) **WF-00 evidence:** agent-kit validator PASS (24/6/10/14); SHA256 verify **33/33 OK, 0 mismatch, 0 missing**; baseline = 1d643ef on main; worktree list recorded (2 reference worktrees, untouched). (3) **Owner decisions recorded:** runtime mark = existing brand PNGs (ATLAS-03), new logo-polish/ SVG candidates + 3 concepts are REVIEW-ONLY pending owner visual gate 16�256px (SOURCE-NOTE + manifest updated); **default theme = system** post-ATLAS-01 (manifest Dark row). (4) **A1 local stack LIVE:** DEV-00 files (infra/cms/docker-compose.local.yml project 	aha-local, infra/deploy/dev-local-stack.ps1 up-db/up-a2/down/reset-db) + BK-L0 config/settings/local.py (DATABASE_URL?Postgres 127.0.0.1:15432, psycopg 3.3.4, stdlib URL parse mirroring production style; development.py sqlite stays as zero-docker fallback). Bring-up: compose config OK ? db healthy ? migrate full chain incl. content.0001�0016 [X] ? superuser localadmin created non-interactively ? runserver smoke on :18000: /admin/login/ 200, /health/ 200, /api/articles/fa/ 404 honest-empty on fresh DB ? canonical pytest -q **393 passed**.
- Why: User instructed the four real blockers (�???) be executed: grouped commits, successful fetch/rebase-equivalent sync, WF-00 baseline, and the parallel DEV-00+BK-L0 laptop stack.
- Scope / files: merge + 5 follow-up commits touching governance/contracts/plan/track docs, .opencode/agent/s-executor.md, WORK_LOG union, Assets/.../brand/{SOURCE-NOTE,logo-polish/**,logo-redesign-concepts/**}, NEW infra/cms/docker-compose.local.yml, NEW infra/deploy/dev-local-stack.ps1, NEW pps/cms/config/settings/local.py. No pps/web or pps/cms runtime source changed (local.py is settings-only, dev-only module not imported by test/production).
- Commands or actions actually performed: fetch + cherry/diff divergence analysis ? 3 commits (b9f738e/2497436/ca58671) ? backup branch ackup/pre-sync-2026-08-26 ? git merge origin/main --no-commit ? conflict policy (ours for authored governance/docs; theirs for origin-only runtime/history; WORK_LOG scripted union) ? rerere recorded ? merge commit ? targeted ADR-0032 fact patches (23 OK / verified anchors) + waves note + repaired plan-index fragment ? validator PASS ? push �3 ? hash verify loop (Get-FileHash) ? compose config ? up-db ? migrate/showmigrations ? createsuperuser --noinput ? runserver bg + Invoke-WebRequest �3 ? pytest.
- Verification actually performed and result: all outputs embedded above; post-merge validator PASS proves reference integrity survived the merge; 393-passed suite proves BK-L0 did not disturb canonical settings; dev-local PS1 runs under Windows PowerShell 5.1 (no pwsh dependency).
- Decisions / assumptions: merge (not rebase) chosen � 34-commit divergence contained parallel runtime evolution (pps/admin), making single-policy merge safer than replaying superseded commits; rationale recorded per user's "fetch/rebase ????" intent (successful sync). Astro pin decision recorded: rebuild locks the current ^7.2.4 / ~5.9.3 / 	ailwind ^4.3.3 lines observed in pps/web/package.json post-merge (manifest policy: exact pins live in the WF scaffold lockfile, not here). /api/articles/fa/ 404 on an empty fresh DB is the documented honest/fail-closed behavior, not a wiring fault (200s on admin+health prove the Django?Postgres path).
- Deferred or risk IDs: none opened. localadmin/DB password are throwaway local-only values inside gitignored-reach files? � they are written IN tracked dev files by design (compose.local.yml/local.py defaults); production secrets remain env-only per base/production settings.
- Rollback / recovery: main pre-sync state on ackup/pre-sync-2026-08-26 (a95931c+3); remote fast-forward history intact on origin; local stack teardown dev-local-stack.ps1 down / 
eset-db; BK-L0 removal = delete local.py (nothing imports it outside local runs).
<<<<<<< HEAD
## LOG-0267 — 2026-08-26 — Budget clauses reclassified as smell thresholds with explicit completeness-over-budget protocol

- Outcome: The per-file line figures across all four track doctrines are no longer interpretable as hard caps that could push small agents to truncate work. TRACK-WF §7.2(4) rewritten ("SMELL THRESHOLDS, not hard caps"; blank lines/imports/frontmatter excluded from counts) and new §7.5 added: absolute priority `feature completeness > a11y/i18n/no-JS contracts > budget`; over-threshold requires exactly one of A (split at a NATURAL seam — artificial micro-splits rejected) or B (ship complete + TECH_DEBT id with size/reason/review-trigger = DONE; silent oversize or half-split = PARTIAL); budget arithmetic explicitly declared never-an-acceptance-criterion (149 lines with skipped keyboard fixture still FAILs; +N lines with all gates passing passes); small-agent escape hatch "if A-vs-B unclear → do B + ESCALATE". Mirrored guard-clauses appended at the three other threshold sites: BK §8.1 view-guidance (~15 lines) and AB §doctrine HTTP-thin (~40 lines) now state validation branches/audit/error-code tests must never be dropped for length, with WF-§7.5 protocol reference; AF screen figure (~300 lines) clarified that legitimately multi-region screens may exceed via natural seams (reducer/dialog extraction), states/keyboard/error paths untrimmable since doctrine gate rows 2/6 would block the packet regardless.
- Why: Owner raised the correct operational risk: a numeric budget read as a cap by literal-minded agents produces broken or incomplete deliverables (dropped ARIA, missing i18n keys, removed tests, TODO stubs) — worse than an oversized file.
- Scope / files: four edits, one per track doc (`TRACK-WF…` §7.2(4) replacement + §7.5 insertion after the Doctrine-compliance line; one-sentence additions in `TRACK-BK…` §8.1(1), `TRACK-AB…` doctrine ¶3, `TRACK-AF…` anatomy ¶2) plus this entry. No runtime/source change.
- Verification actually performed and result: WF edit anchor matched uniquely on second attempt (first attempt failed due to em-dash rendering mismatch; re-read lines 135–139 then applied against exact bytes); BK/AB/AF anchors matched first try; resulting §7.5 sits between the doctrine-compliance rule and end-of-file so packet tables above remain untouched.
- Decisions / assumptions: kept the numbers themselves (150/200/80/15/40/300) rather than deleting them — they remain useful tripwires; only their enforcement semantics changed. Chose TECH_DEBT+B as the default safe path for undecided agents because shipping complete code with a logged debt beats both truncation and risky last-minute splits.
...
- Rollback / recovery: revert the five paragraph-level edits in the four docs and drop this LOG-0267 entry; nothing else references §7.5 outside these files.

## LOG-0266 — 2026-08-26 — Engineering-doctrine sections added to all four track lists (component-driven / scalable / refactor-friendly mandates)

- Outcome: Four track task lists now carry a binding, per-packet-enforced engineering doctrine so all small agents build principled, scalable, component-driven code with easy future fixes: TRACK-WF §7 (layer model tokens→primitives→content→templates→routes with strict ascending imports, single-source-of-truth closed 24-registry, props-in/markup-out purity, slots-not-config, a11y-as-API; scalability rule "new route family = template+adapter+spec only"; token-only magic values with QA enforcement; file-size budgets ~150/200/80 lines; horizontal theming/RTL via role selectors + logical properties; 10-row mechanical gate table incl. raw-value grep, style-block ≤40 lines, i18n ZWNJ byte-exactness, import hygiene, bundle discipline; extension-seams documentation at declaration site, delete-before-diverge, change-surface predictability matrix retheme/new-section/IA/CTA). TRACK-BK §8 (Model→Service→Projection→View layering with view-code ≤~15 lines before service extraction; one-concept-one-model via BK-DATA-CONTRACTS closed registry; DTO keys append-only synced to doc in same commit; migration hygiene descriptive-named reversible single-change; `_fa/_en` naming locked; indexes/constraints same-commit; lifecycle-mixin reuse instead of new status columns; query-count assertion pattern for N+1; gates table incl. migration-drift, leak-negatives, doc-parity, fat-view grep; extensibility seams §8.4: projection-layer plug points for future FTS/vector without model reshaping). TRACK-AB doctrine (one-domain-one-module routers cross-importing only admin_common; endpoints-additive-only with ADR-gated reshape path; HTTP-thin views ≤~40 lines extracting pure functions; uniform session+CSRF+TOTP+audit `<domain>.<verb>`; v2 namespace growth instead of v1 mutation; validator-as-library stable-token pattern; contract-first both-docs sync on drift; test pyramid unit-table/integration/curl with every declared error code tested; gates incl. zone-purity diff stat, auth-parity grep, error-code registry sync, OpenAPI operation-id match). TRACK-AF doctrine (three-layer React anatomy ui-kit→domain-components→screens with ~300-line screen split bar; reducer/state-machine-first for multi-step flows blocking useState webs — Home Composer pattern canonical; fetches only through typed clients with ProblemDetails tokens to i18n mapping never raw-rendered; keyboard reachability+focus trap+label-binding+non-color-meaning built-in; zero-new-runtime-deps default; i18n complete-at-commit with Appendix A byte-exact copy; RTL physical-property zero-tolerance maintained; deletion-friendly growth tied to MODE §7 with route-registry no-dangling-import proof; server-cache vs editor-draft separation enabling future undo/autosave; gates incl. reducer-tests-present, dependency-count check, dict-parity, zone purity, keyboard-only pass recorded).
...
- Decisions / assumptions: doctrine sections deliberately follow per-track wording (frontend composition vs backend service-layer vs API contract vs React reducer) because forcing one universal checklist would blur ownership lines established in MODE §8; the shared elements enforced everywhere are: layered architecture, single-source-of-truth registry, additive-only contracts, tests-in-same-commit, no silent dependency/string invention, deletion-with-adoption, and the `Doctrine compliance:` log suffix which makes small-agent output auditable line-by-line.
...
- Rollback / recovery: revert the four section appends (files are working-tree additions; `git checkout` not applicable until first commit — restore = remove appended sections or delete untracked docs) and drop this LOG-0266 entry. No runtime impact.

## LOG-0265 — 2026-08-26 — Four-track conflict-free task lists + dev environment modes + clean-cutover policy

- Outcome: Execution plan split into four non-overlapping tracks with per-packet agent-grade detail, plus a shared environment/cutover contract: (1) `docs/plan/TRACK-MODE-environment-and-cutover.md` — running modes S(SNAPSHOT)/A1(LOCAL host-CMS+docker Postgres `:15432`/`runserver :18000`)/A2(fully dockered `:18001`)/B(HYBRID front-local + prod-backend read-only ≤30req/min), one-time packet DEV-00 (`infra/cms/docker-compose.local.yml` project `taha-local`, `infra/deploy/dev-local-stack.ps1` up-db/down/reset-db) and BK-L0 (`config/settings/local.py`), seed-data options incl. owner-gated prod restore, clean-codebase policy §7 (adopt-equals-delete rule, `apps/web/LEGACY-INVENTORY.md` mechanics with ephemeral `npx unimported`/`npx depcheck`, BK drop-ticket protocol, post-cutover owner checklist), final cross-track conflict matrix §8. (2) `TRACK-AB-admin-backend-api-task-list.md` — 8 packets AB-00..07: baseline url marker `apps/api/admin_urls.py` + guarded `[ADMIN-API]` include, shared If-Match/ProblemDetails helpers, frozen endpoint contracts for home-modules (GET/PUT-bulk+If-Match/validate), timeline CRUD+reorder, media presentation PATCH (focal % / rights locale / license list), graph versions (draft→activate atomic, payload GraphNodePublic camel shape) and pure validator module with stable issue tokens; every packet pytest negatives ≥10 where stated, curl roundtrip evidence on A1 stack. (3) `TRACK-AF-admin-spa-task-list.md` — 10 packets AF-00..07+CLEAN: typed client `adminApiExt.ts`, Home Composer reducer-driven (keyboard ▲▼ reorder, no dnd dep, revision-conflict dialog shared component), Timeline editor with `useOrderedDraftList` hook, MediaPicker focal crosshair (% clamp/round) + license select cache, hand-rolled SVG graph editor shell (wheel-zoom cursor-centered clamp 0.25–3, DPR-aware, SR fallback list cap 200, keyboard nudge ±1/×10), validation UX polling AB-06 tokens with `GRAPHISSUE_TOKENS` sync-guard test, semantic-list preview drawer, EXPLICIT DEFER recorded for embedded site iframe preview, i18n Appendix A Persian strings byte-exact, RTL grep gate `(left|right):`, zero-new-runtime-deps rule. (4) Appended to existing lists: TRACK-WF gains WF-CLEAN + §5b environment preconditions (SNAPSHOT default → A1 from adoption; hybrid read-only discipline); TRACK-BK gains BK-L0 pointer + §7 field/column drop protocol; `docs/plan/README.md` new §1b four-track board v2 mapping to ATLAS numbering with G0..G9 unchanged, sequencing line DEV-00+BK-L0 → ∥(WF-00..06 ‖ BK-00..04) → AB → ∥(AF ‖ WF-07*) → BK-05 → WF-08 → QA/CLEAN → owner cutover.
- Why: Owner requested (a) directory arrangements ensuring no-longer-used code is easily deletable after the rebuild replaces the old frontend/backend surfaces, (b) ready-to-run laptop testing with backend+database locally OR frontend-local/backend-remote as alternatives, (c) four task lists (backend, frontend, admin-backend, admin-frontend) detailed enough for small agents, targeting a ground-up frontend rebuild plus required backend/admin capabilities — all without track collisions.
- Scope / files: three NEW docs (`TRACK-MODE-environment-and-cutover.md`, `TRACK-AB-admin-backend-api-task-list.md`, `TRACK-AF-admin-spa-task-list.md`), two APPENDED docs (`TRACK-WF-…task-list.md` +WF-CLEAN row & §5b; `TRACK-BK-…task-list.md` +§5b/§7), one INDEX edit (`docs/plan/README.md` §1b inserted before §2 Queued), this entry. No runtime/source files touched: `apps/**`, `infra/**` untouched (DEV-00/BK-L0 artifacts are future packet outputs, documented not executed here).
- Commands or actions actually performed: re-read MODE-relevant repo facts (`docker-compose.cms.yml` fail-closed envs motivate separate `.local.yml`; runserver pattern from canonical commands; RATE_LIMITED vocabulary from LOG history), verified all five target docs exist via Test-Path (all True), confirmed highest WORK_LOG id `0264` via Select-String scan → allocated next ID `0265`.
- Verification actually performed and result: file existence checks PASS ×5; README insertion anchor matched uniquely (edit applied); ownership matrix in MODE §8 cross-checked against AGENT-COORDINATION.md §3 zones and prior ADR-0026 separation statements — no zone assigned to two writers; endpoint paths in AB declared once and referenced verbatim by AF; AF references only TS zone + reads AB doc (no python file writes).
- Decisions / assumptions: local Postgres port chosen `15432` and compose project `taha-local` to avoid any collision with prod names/ports (`taha-cms`, `18000` host exposure remains a host-mode choice inside A1 via runserver). Local DB credentials are throwaway values written openly in the dev-only files, never reused for prod; no secret added to git. `npx unimported|depcheck` treated as ephemeral tooling (no package.json dependency) consistent with no-new-deps governance. Embedded site preview from admin deferred honestly rather than half-built (MASTER-SPEC §8 gates remain public-side/atlas scope); DEFER id to be formally allocated at AF-06 execution time per ledger protocol. ATLAS single numbering kept as reference mapping to avoid rewriting already-reviewed spec text; four-track board is the execution superset.
...
- Rollback / recovery: delete the three new docs, revert the two appends and the README §1b insert (`git checkout HEAD -- <paths>` works only if they were never committed — they are currently untracked/unstaged working-tree changes; restoring = remove files / revert edits manually), then drop this LOG-0265 entry. No runtime, schema, dependency, infra or production rollback applies because nothing outside `docs/plan/**` changed.

## LOG-0264 — 2026-08-26 — Align all docs/contracts/configs to site-redesign implementation-reference as canonical frontend source

- Outcome: All upstream docs, contracts and tooling configs rewritten to align with `Assets/site-redesign/implementation-reference/` (branch `p14c-visual-atlas`, commit `7d9b87f`) as the canonical next-generation frontend brief (planning/handoff, not live). Public frontend will be **rebuilt from scratch in `apps/web/` as Astro 7 + TypeScript 5.9 + Tailwind v4 + React 19 islands** per `MASTER-SPEC.md` + `agent-kit/*.json` + `AGENT-COORDINATION.md` + `MULTI-AGENT-TASK-LIST.md` (ATLAS-00..12) + `ACCEPTANCE-GATES.md` (G0..G9). **Frontend/admin separation invariant (ADR-0026) preserved:** `apps/web` (public Astro, `DESIGN_ATLAS=1` → `/_design/` local-only) remains a separate project/build/route from `apps/cms` + `apps/cms/admin-frontend` (React SPA at `/admin/` + Django staff at `/staff/`); no shared writable worktree, no merged bundle — any merge proposal must be rejected. No P14 runtime, CMS schema, route, dependency lockfile, infrastructure, CI deployment, or production-content change occurred in this doc-alignment pass.
- Why: Owner requested the `site-redesign` folder (canonical path `Assets/site-redesign/`) become the single frontend reference, all previous docs/contracts rewritten to match it, frontend rebuilt from scratch, and the frontend↔admin separation decision retained. Allowed multiple sub-agents for speed/quality.
- Scope / files:
  - Backup (pre-rewrite, 2026-08-26_08-37-10): `docs/` (full), `PROJECT_MANIFEST.md`, `AGENTS.md`, `README.md`, `Task-list.md`, `reDesign_plan.md`, `apps/web/src/styles/global.css`, `apps/web/package.json` + `astro.config.mjs`, `infra/`, `.opencode/`, `.github/`, plus snapshot `site-redesign-reference-snapshot/` + `_worktrees_snapshot.txt` → `_archive/pre-redesign-backup_2026-08-26_08-37-10/` (4385 entries, `_BACKUP_MANIFEST.txt`).
  ...
  - Rewritten (visible in `git diff --stat HEAD`, 19 tracked paths + 1 untracked `docs/plan/ATLAS-frontend-rebuild-task-spec.md`):
    - Governance: `AGENTS.md` (Current gate → P14 ATLAS, ownership adds implementation-reference as brief-only, Visual Atlas `DESIGN_ATLAS=1` launcher, `injectRoute` in `astro.config.mjs`, `node agent-kit/validate.mjs`).
    ...
    - Entry: `docs/README.md` (read order now AGENTS → MANIFEST → implementation-reference README→MASTER-SPEC→agent-kit → Task Spec → contracts; What-is-true table adds P14 rebuild NOT STARTED row, history note for `design.md`/`user-journey`, source-of-truth map adds next-gen vs runtime rows, Directory map adds implementation-reference).
    - Contracts: `docs/contracts/IA-CONTRACT.md` (Live vs Target ATLAS navigation split — live retains `/{locale}/writing/` + P8 catalogs + search + blog→writing redirect + More▾; target MASTER-SPEC §4 About·Research(Publications child)·Projects·Gallery→`/creative/`·Blog→`/writing/`·Learning→`/teaching/` + utilities; homepage minimum Live vs Target §5 1–8; Search visibility now live Pagefind) + `docs/contracts/DESIGN-CONTRACT.md` (authority `global.css` + `tokens.json` Light runtime / Dark design-target, stack boundary Astro7/Tailwind/React islands + motion/gsap/three bounded to ATLAS-10, tokens tables Light `#f7f8f5`/`#182328` + Dark `#071225`/`#f7f3ea`, 6 widths 320–1440, spacing 4–96px, glass restricted, Atlas isolation G4, 24 primitives listed).
    - History: `docs/design.md` (superseded → HISTORY, 3575→~54 lines, migration table to `tokens.json`/`components.json`/`templates.json`/`MOTION-GRAPH-HANDOFF` etc, MASTER-SPEC outranks `reDesign_plan.md` §12–13, backup pointer) + `docs/user-journey-information-architecture.md` (HISTORY, 3910→~56 lines, 6 journeys PhD supervisor/academic reviewer/industry reviewer/reader/learner/creative viewer, migration old `/work`/`/teaching`/`/creative` → new families).
    - Plan: `docs/plan/README.md` (active ATLAS timeline ATLAS-00 READY … ATLAS-12 BLOCKED with Owner/Depends/Gate G0..G9, CMS audit G6 before migrations, reference validation gate) + new `docs/plan/ATLAS-frontend-rebuild-task-spec.md` (umbrella READY but per-packet specs required; Objective/Context/Non-goals/Scope/Dependencies/Acceptance G1..G9/ Risks) + `Task-list.md` header + Global Constraints rewritten to ATLAS-aligned English (Goal includes rebuild from scratch + separation invariant, note that active frontend order is `docs/plan/README.md` ATLAS board).
    ...
    - Tooling (tracked): `.opencode/agent/s-executor.md` (ATLAS executor, allowlist adds `apps/web/qa/**`, `apps/web/scripts/**`, `docs/contracts/**`, `docs/status/deferred-validation.md`, `Assets/site-redesign/…`, bash allow `git worktree*` + `node agent-kit/validate.mjs`) + `.opencode/agent/visual-reviewer.md` (ATLAS visual QA, 9-criterion checklist vs MASTER-SPEC/tokens/DESIGN-CONTRACT/G1..G9, tokens `#f7f8f5`/`#071225` etc) + `.opencode/opencode.json` (added `_atlas` note, ignored via `.gitignore` so not in `git diff`) + `.github/workflows/ci.yml` (added Reference integrity `node agent-kit/validate.mjs` G0 + Atlas isolation `no /_design/` G4 after Build). `.cursor/rules/*` (00 planning→`Task-list.md`+`docs/plan/README.md` ATLAS board + G0..G9, 01 documentation-sync→MASTER-SPEC+agent-kit+DOCUMENT-MIGRATION-MAP, 02 risk→`docs/status/RISK_REGISTER`+`TECH_DEBT`+`deferred-validation`+`known-issues`+`WORK_LOG`, 03 git→ATLAS worktree model `AGENT-COORDINATION.md` §3 ownership) + `.cursor/agents/*` (system-architect, design-system-guardian, verifier tuned to ATLAS tokens/layout/glass/gates) rewritten but `.cursor/` is gitignored via `.gitignore:59` so not in `git diff` (verified via `git check-ignore`). `.serena/project.yml` (added ts/html/scss/json language servers, ignored_paths for `node_modules/dist/.astro/.venv/infra/backup`, initial_prompt ATLAS brief) also gitignored via `.gitignore:80`.
    - Assets (upstream P14C changes now visible as unstaged vs old HEAD `a95931c`): `Assets/site-redesign/README.md` (2026-08-26 visual atlas reviewed, +page-families row), `Assets/site-redesign/MANIFEST.md` + `SHA256SUMS.txt` + `PROMPTS.md` updated with 8 page-family PNGs (hashes `b26103…`..`72d547…`). These are P14C handoff artifacts already in `Assets/site-redesign/implementation-reference/` and mirrored to the parent pack; they are not invented here.
- Commands or actions actually performed:
  - Backup: `Copy-Item -Recurse` docs/manifest/AGENTS/README/Task-list/reDesign_plan/global.css/package.json/astro.config/infra/.opencode/.github → `_archive/pre-redesign-backup_2026-08-26_08-37-10/` + snapshot `site-redesign-reference-snapshot/` (4385 entries) + `_BACKUP_MANIFEST.txt`; extended copy for `.cursor`/`.serena`; `git worktree list` → `_worktrees_snapshot.txt`.
  - Delegated parallel rewrites via task tool (4 sub-agents): AGENTS+README, PROJECT_MANIFEST, IA+DESIGN contracts, design+user-journey history, plan/README+ATLAS spec — each verified via re-read after write.
  - Direct edits (continuation after 08:37): `IDEA.md` (16→32 lines), `.opencode/opencode.json` (+`_atlas`), `.github/workflows/ci.yml` (+2 jobs G0/G4), `Task-list.md` header+Global Constraints (ATLAS-aligned English), plus `.cursor`/`.serena`/`.opencode/agent` rewrites (verified via `Get-Content` + `git check-ignore`).
  - Verification: `node Assets/site-redesign/implementation-reference/agent-kit/validate.mjs` → **PASS** (`24 components, 6 templates, 10 asset references, 14 reference files, offline Figma builder`); `git diff --check` → no whitespace error (only LF→CRLF warning on `SHA256SUMS.txt` via lightningcss/normalization, not a content error); `git status --short --branch` → `main...origin/main [ahead 6, behind 34]` with 19 tracked modified + 1 untracked ATLAS spec; `Test-Path` backups → True for all 4 critical paths.
...
- Decisions / assumptions: `Assets/site-redesign/` on disk already equals the requested `site-redesign` reference (Windows path `D:\Project\Taha-personal-platform\site-redesign` does not exist; the asset pack is `Assets/site-redesign/` with `implementation-reference/` at `7d9b87f`). That package is the brief, not a live deployment — no frontend rebuild started. For conflicting visual rules, `MASTER-SPEC.md` outranks `reDesign_plan.md` §12–13 per `AGENTS.md`/`docs/README.md`. `Assets/site-redesign/others/` remains unmanaged owner drop (outside hash contract). `.worktrees/` and `.cursor`/`.serena`/`.opencode` (except `agent/`) remain gitignored by design; their rewrites are local-tooling hygiene and do not enter the deploy artifact. Frontend/admin separation is preserved exactly as `PROJECT_MANIFEST.md:74` + `AGENTS.md` ownership + `AGENT-COORDINATION.md` §3 (exclusive `global.css`, `Header.astro`, `BaseLayout.astro`, `astro.config.mjs`, `package.json`, ledgers).
- Deferred or risk IDs: none opened. ATLAS packets themselves remain separately gated (ATLAS-08 audit before any CMS migration G6, no production deploy without owner approval + backup/rollback + CI + smoke per `AGENTS.md` CI gate). Remaining `.github/workflows/cd.yml`/`ci-cms.yml` Caddy/Compose deploy paths unchanged and still assume legacy host `/opt/taha/site/current` fallback plus Compose edge `WEB_IMAGE` derive — no change here. Infra timers, EMAIL_host, and `content.0015/0016` already on prod remain operator-owned as before.
...
## LOG-0263 — 2026-08-26 — Frontend implementation architecture blueprint
...
- Ledger: `known-issues.md` KI-0007 → CLOSED. No code change required.

## LOG-0246 — 2026-08-24 — ADR-0031 «Glass Constellation» accepted + governance sync (contracts/manifest/ledgers/board)

- Outcome: **تصمیم بازطراحی v2 حاکمیت یافت.** مالک جهت جدید را attestation کرد: هویت dark-first شیشه‌ای («Glass Constellation»)، هیرو متحرک Three.js با واکنش موس، گراف موضوعات D3، Journey با GSAP، تعامل‌های pointer (spotlight/tilt/magnetic)، landing ترغیب‌کننده، و انضباط عرض متن/فضای منفی با یک الگو. `docs/adr/0031-glass-constellation-redesign.md` ACCEPTED: فلسفهٔ سطح «Solid-first + Selective Glass» design.md §12–14 و نسخهٔ ۱ سند بازطراحی (کاغذی) را supersede می‌کند؛ معماری Astro+islands و تمام قراردادهای IA/CMS/SEO دست‌نخورده می‌مانند. `reDesign_plan.md` → **v2** بازنویسی شد (توکن‌های شب §0، دستورالعمل پنج‌بخشی شیشه، نردبان حرکت M0–M4 با بودجه‌ها، لایهٔ primitives/ui/patterns/sections/islands، فازهای ۰–۵). همگام‌سازی: `DESIGN-CONTRACT.md` v2 (§0 night tokens + §0b component layer + §2 legacy-marked + §2b glass v2 + §3b motion ladder + Hover-AA rule جای hover-darker + DoD به‌روز)؛ `design.md` status block (v2 in flight؛ light tables = تاریخچهٔ صفحات مهاجرت‌نشده)؛ `PROJECT_MANIFEST.md` (Styling/UI row = component layer ADR-0031؛ Motion/GSAP/Three/D3 rows = AUTHORIZED با بودجه؛ Dark mode row = SUPERSEDED single-night-theme)؛ `deferred-validation.md` DEFER-0025 → CLOSED (superseded/WONTFIX-dual-theme)؛ board: F4 `[x]` superseded، WS-R جدید (R0–R5) اضافه، M1 milestone شامل فازهای ۰–۲، tracker row DEFER-0025 بسته؛ `IA-CONTRACT.md` §9 ترتیب خانهٔ v2 (فعال شدن تدریجی با فازها).
- Why: درخواست مالک پس از attestation سبک — «اول تمام قراردادها و داکیومنت‌های مرتبط را با تصمیمات جدید به‌روز کن».
- Scope / files: `docs/adr/0031-glass-constellation-redesign.md` (جدید)، `docs/adr/README.md` (+0030/0031 rows، یادداشت 0028)، `reDesign_plan.md` (v2 rewrite)، `docs/contracts/{DESIGN-CONTRACT,IA-CONTRACT}.md`، `docs/design.md` (status block فقط)، `PROJECT_MANIFEST.md` (۳ ردیف جدول‌ها)، `docs/status/deferred-validation.md` (DEFER-0025)، `docs/plan/master-remaining-work-checklist.md` (F4، tracker، WS-R، M1)، این entry. هیچ فایل کد تغییر نکرد.
- Commands or actions actually performed: docs-only edit pass؛ بدون build/دستور کد.
- Verification actually performed and result: cross-check دستی مراجع متقابل (ADR↔contract↔manifest↔ledger↔board) — هیچ تناقض باز درباره تم/حرکت/لایه کامپوننت باقی نماند؛ IA §4 nav (More▾ از LOG-0245) دست‌نخورده.
- Deferred or risk IDs: DEFER-0025 CLOSED (superseded)؛ اجرا در R0–R5 هر یک WORK_LOG خودش را می‌گیرد؛ KI-0007 همچنان OPEN.
- Rollback / recovery: docs-only؛ revert git هر فایل مستقل است؛ ADR-0031 با ADR جدید قابل نقض است نه ویرایش.

## LOG-0245 — 2026-08-24 — Migrate attended PASS (content.0015/0016) + CD serialization fix + P9 bridge flip + E9 More nav

- Outcome: **migrate پروداکشن با موفقیت انجام شد.** dispatch اول مالک (#103) سه فیل داد: (1) سه job gated موازی همزمان روی یک working tree VPS عملیات git کردند → `Cannot fast-forward to multiple branches` / fetch corruption؛ fix: زنجیره `needs` + `!cancelled()` در `cd.yml` (migrate → rebuild-web → timer) — `5790234`. (2) `install-scheduled-timer` به sudo تعاملی نیاز دارد (NOPASSWD فقط update-release/caddy-apply) — همان Infra#9؛ دستور one-time مالک در کامنت job مستند شد. (3) dispatch دوم (#105) فیل جدید: فیکس fail-closed `${WEB_IMAGE:?}` با `update-cms.sh` تداخل داشت (فقط CMS_IMAGE export می‌کرد)؛ fix: derive WEB_IMAGE از کانتینر running — `4e064f9`. **dispatch سوم (#106, run 32705516385) PASS کامل:** backup 383816B → `Applying content.0015_p9_courses_creativeworks... OK` + `content.0016_collection_and_taxonomy_glossary... OK` → CMS روی `taha-cms:3ce3fa9` → smoke-cms 9/9 PASS → rebuild-web با کد جدید (taha-web:local از main@4e064f9) → smoke 9/9 PASS. `showmigrations content`: `[X] 0015 + [X] 0016` تایید SSH. **پیامد:** پنجره web-ahead-of-CMS بسته شد — دو bridge 404 در `creative.ts`/`courses.ts` به throw فلیپ شدند (کامنت bridge removed). **E9 (DEFER-0037 CLOSED):** تصمیم مالک گزینه B — گروه More در هدر: `<details class="nav-more">` بدون JS (کیبورد/تاچ native) با Publications/Books/Talks/Downloads، caret SVG، `aria-current` روی گروه وقتی فرزند current است، پنل dropdown با توکن‌ها؛ ترتیب هدر: About · Research · Projects · Writing · More▾ · CV · Contact · Search · lang. `IA-CONTRACT.md` §4 قرارداد nav به‌روز شد. Board E9 `[x]`. **نکته عملیاتی:** `/health/` حالا `degraded/contact:error` گزارش می‌کند (فرم تماس فعال، EMAIL_HOST خالی) — رفتار درست E7/Infra#8؛ رفع آن = ست کردن EMAIL_*/CONTACT_FORM_TO در `infra/cms/.env` VPS توسط مالک.
- Why: مالک dispatch را انجام داد و خواست «درستش کن، خودت هم دیپلوی کن» + E9 گزینه B.
- Scope / files: `.github/workflows/cd.yml`، `infra/deploy/update-cms.sh`، `apps/web/src/lib/cms/{creative,courses}.ts` (flip)، `apps/web/src/components/Header.astro` (More group)، `docs/contracts/IA-CONTRACT.md`، `docs/status/{deferred-validation,WORK_LOG}.md`، `docs/plan/master-remaining-work-checklist.md`. Commits: `5790234`, `4e064f9`, این push.
- Commands or actions actually performed: `gh run view --log-failed` ×2، edit cd.yml (needs chain) + yaml validate، edit update-cms.sh derive + `bash -n`، dispatch `gh workflow run cd.yml -f migrate_cms=true -f rebuild_web=true -f cms_image_tag=3ce3fa9` ×2، monitor تا PASS، SSH showmigrations، flip bridgeها، More dropdown + `npm run check/build` + 4 QA spec PASS.
- Verification actually performed and result: run 32705516385 همه jobs سبز (migrate + rebuild-web)؛ showmigrations 0015/0016 `[X]`؛ CMS image 3ce3fa9 live؛ web rebuilt از main؛ build 58p؛ specs (research-graph/p8/sitemap/health) PASS؛ dist شامل `<details class="nav-more">` با ۴ لینک.
- Deferred or risk IDs: Infra#9 timer = one-time owner sudo روی VPS (دستور در cd.yml کامنت)؛ EMAIL_HOST خالی → health degraded (owner env)؛ KI-0007 OPEN.
- Rollback / recovery: migrate: `export CMS_IMAGE=705ed4c && bash update-cms.sh` (backup موجود)؛ migrationها additive با reverse؛ bridge flip با restore کامنت 404؛ More group با حذف details block.

## LOG-0243 — 2026-08-23 — E2/E4/E10 + ADR-0030 + Three.js signature + board sweep — WS-E کامل (جز E9 owner)




- Outcome: ادامه اسپرینت دیزاین پس از مجوز مالک (ADR-0030). **حاکمیت:** `docs/adr/0030-visual-interaction-libraries.md` ACCEPTED — motion/gsap(license)/d3/three مجاز؛ three با بودجه صریح ≤150KB gzip lazy برای یک signature experience؛ B5 candidates 1-3 → authorized + non-goal superseded؛ LICENSES.md gsap acceptance ثبت. **E2 (KI-0003/4/5 CLOSED):** `--color-brand-emphasis-hover #0a6a62` (5.4:1) در hoverهای Landing/ContactPage/Footer؛ `--color-control-border #748682` روی `.cta--ghost`؛ `--color-ink-secondary #616e74` (4.54:1 روی muted)؛ قرارداد hover-darker rule + دو token row. **E4:** gold marks سطوح روشن (Landing rules/constellation، About) → `--color-signature`؛ gold-500 روی navy معیار ماند. **E10:** 24 اسکرین‌شات قبل(live)/بعد(build جدید) در `docs/status/evidence/e10-design-elevation/` — gateway/home(3D)/search/publications/teaching × desktop/mobile با `qa/e10-evidence-shot.mjs`. **Three.js signature (ADR-0030 Candidate 5):** `Constellation3D.tsx` island — 5 node فیروزه‌ای + core طلایی + spokes، pointer-parallax damped، `client:visible`، lazy `import("three")` → chunk **128.7KB gzip ≤ 150KB**، mobile<768px → static SVG، reduced-motion → یک فریم ثابت، WebGL fail → fallback SVG، dispose کامل. **Candidate 1:** constellation entrance CSS-only (stroke-draw staggered، reduced-motion static، 0 JS). **Candidate 3 Journey:** SKIP صحیح — محتوا/markup وجود ندارد (بدون اختراع محتوا)؛ gsap authorized ولی تا Task Spec محتوایی import نشده. **Board sweep:** E2..E8+E10 → `[x]` با شواهد؛ E9 (P13 findability) owner-decision باقی. **سایر:** E3 (LOG-0240 batch)، E5/E7 (LOG-0242)، E8/S35 (LOG-0241)، E6-P17/S25/S27 قبلاً بسته.
- Why: درخواست مالک «تا پایان تسک‌های دیزاین ادامه بده» + مجوز صریح کتابخانه‌های تعاملی.
- Scope / files: `docs/adr/0030-*.md`، `docs/plan/{B5-*,LICENSES.md}`، `apps/web/src/styles/global.css`، `apps/web/src/components/{Landing,About,ContactPage,Footer,Header}.astro`، `apps/web/src/components/islands/Constellation3D.tsx`، `apps/web/qa/e10-evidence-shot.mjs`، `docs/contracts/DESIGN-CONTRACT.md`، `docs/status/{known-issues,evidence/e10-*}`، `docs/plan/master-remaining-work-checklist.md`، `docs/design.md` (LOG-0241)، این entry.
- Commands or actions actually performed: `Task ×5` (2 network-fail → دستی؛ 3 موفق: E3، motion، 3D؛ سپس E7/E5 و E8/S35)، edit مستقیم tokens/hover/ghost/gold-routing، `npx playwright install chromium`، preview :8901 + `node qa/e10-evidence-shot.mjs` ×2 (after local / before live)، `npm run check` 0 err، `npm run build` 58p، chunk audit (three 128.7KB gzip).
- Verification actually performed and result: build سبز؛ dist شامل `#0a6a62`/`#748682`/`scroll-padding-top`/`.glass-surface` واحد/var reduced-motion؛ three chunk lazy و در بودجه؛ 24 PNG evidence؛ هر 6 workflow قبلی سبز.
- Deferred or risk IDs: E9/P13 OPEN (owner product decision)؛ DEFER-0035 PARTIAL (visual pass)؛ Journey-gsap منتظر محتوای §71؛ KI-0007 OPEN (Cloudflare)؛ `content.0015/0016` migrate owner-attended.
- Rollback / recovery: `git revert` per-commit؛ island با حذف mount از Landing حذف می‌شود (static SVG همیشه fallback)؛ tokenها additive.

## LOG-0241 — 2026-08-23 — E8 design.md honesty (DEBT-0015/DEBT-0001, P19) + S35 unified Breadcrumbs

- Outcome: **E8:** `docs/design.md` now opens with a status block (after the title) declaring the file the DESIGN VISION reference with binding rules = `DESIGN-CONTRACT.md` + `IA-CONTRACT.md` + `apps/web/src/styles/global.css` tokens; in-force list (Astro static-first, CSS/Tailwind + global.css tokens, ADR-0030 motion, Inter+Vazirmatn per ADR-0019 DECIDED, light-only, no mascot/command palette/dark theme) and not-yet-built vision list added. Opening "UI foundation: … shadcn/ui + Radix Primitives" line annotated admin-only; §2.1 Final self-assessment table retired to one line ("Retired 2026-08-23 — see status block."); §159 open-decision items 2–3 (English/Persian typeface) marked DECIDED — ADR-0019 (Inter + Vazirmatn Variable). `TECH_DEBT.md`: DEBT-0015 → CLOSED (2026-08-23, header rewritten), DEBT-0001 → CLOSED (2026-08-23, status block distinguishes vision vs in-force). **S35:** duplicate breadcrumb components unified — canonical = tokenised `apps/web/src/components/Breadcrumbs.astro` (`--space-*`/`--color-ink-*`/`--text-sm`/`--color-brand`, hover/focus-visible + current-page styling; strict superset of the blog variant which used raw rems; identical Props/ARIA so nothing needed porting); all 44 page imports of `blog/Breadcrumbs.astro` repointed to the canonical path (2 about pages already used it); `apps/web/src/components/blog/Breadcrumbs.astro` deleted.
- Why: Board E8/S35 — design.md presented stale vision as current architecture (root cause chain of KI-0002 family) and two breadcrumb components invited drift.
- Scope / files: `docs/design.md` (status block, UI-foundation annotation, §2.1 retirement, §159 items 2–3), `docs/status/TECH_DEBT.md` (DEBT-0001/0015 rows), 44 files under `apps/web/src/pages/{en,fa}/**` (import path only, one line each), `apps/web/src/components/blog/Breadcrumbs.astro` (deleted), this entry.
...
- Verification actually performed and result: `npm run check` → **0 errors** (14 pre-existing hints, unrelated files); `npm run build` → **58 pages PASS** + Pagefind en/fa index; `git grep "Breadcrumbs.astro" apps/web/src` → only `components/Breadcrumbs.astro` (46 imports, one canonical path); dist JSON-LD intact — 42 built HTML pages carry `BreadcrumbList` (emission lives in `apps/web/src/data/structured.ts:134` `breadcrumbJsonLd`, independent of the visual component); `/en/writing/index.html` contains both nav breadcrumbs and JSON-LD.
- Decisions / assumptions: root `Breadcrumbs.astro` chosen over `blog/Breadcrumbs.astro` because it is fully tokenised per DESIGN-CONTRACT and its CSS is a functional superset (hover/focus-visible/current states) with identical markup semantics — deletion safe without porting. Working tree contained concurrent uncommitted B5/E6/ADR-0030 changes from other sessions; left untouched.
...
## LOG-0242 — 2026-08-23 — E7 + E5: primaryColor AA clamp (RISK-0014) + unified glass pattern & overridable reduced-motion (DEBT-0012/13)

- Outcome: **E7 (RISK-0014 CLOSED):** `validateBrandContrast(hex)` exported from `apps/web/src/lib/cms/siteSettings.ts:67` — pure WCAG math, worst-case ratio of the hex against white `#ffffff` and canvas `#f7f8f5`, `ok === ratio >= 4.5`; allowlist `ALLOWED_BRAND_COLORS` (#087c73, #0d9689, #0a6a62, #16b8a6) at `siteSettings.ts:45`. `BaseLayout.astro:50-67`: when CMS build (`isCmsOriginBuild()`) and an override is present, a sub-AA color is clamped to the first allowlisted turquoise that passes and `console.warn`s — owner choice preserved, build never fails. Math verified in isolation: #087c73 → 4.757 (passes), #0d9689 → 3.432 / #0a6a62 → 6.060 / #16b8a6 → 2.334; a failing override (e.g. `#ff0000` or even `#0d9689`) clamps to `#087c73`. No tests added per task spec (verify via build). **E5 DEBT-0012 CLOSED:** single glass pattern `.glass-surface` (+ `--dark` variant) in `global.css` (`global.css:270-291`) — opaque-first solid fallbacks (`--glass-solid-light/--dark`) declared before one shared `@supports ((backdrop-filter…) or (-webkit…))` upgrade block; Header (`Header.astro:41`) drops its local `@supports (backdrop-filter)` block and Gateway (`pages/index.astro`) drops its local backdrop rules + `@supports not` fallback, both now consume the utility with visual values unchanged (header blur 10px/sat 120% over canvas↔0.82-alpha light glass; gateway blur 16px over navy-900↔dark glass + border/shadow). Glass blur tokens aligned to live surfaces (`--glass-blur-light:10px`, `--glass-blur-dark:16px`, `--glass-saturate:120%`; old unconsumed `--glass-blur:14px` removed). **E5 DEBT-0013 CLOSED:** global reduced-motion override replaced by the overridable pattern `animation-duration: var(--reduced-motion-duration, 0.01ms) !important` + transition twin (`global.css:195-208`); hover-lift clause struck from DESIGN-CONTRACT §3 with strike-note (no hover-lift transforms exist), §6 checklist item generalized.
- Why: Board E7/E5 — RISK-0014 required build-time contrast safety on the CMS brand override without fail-build; DEBT-0012/13 required one documented glass pattern and honest motion contract.
...
- Verification actually performed and result: dist CSS `global.B9vfX5ui.css` contains exactly ONE glass `@supports` (minified to `(backdrop-filter:blur(1px))`; other @supports hits are Tailwind @layer-property hacks) and no `-webkit-backdrop-filter` outside it (lightningcss minification strips prefixes — same pipeline behavior as before this change); reduced-motion block carries `var(--reduced-motion-duration,.01ms)!important`; `/en/about/` header renders `class="site-header glass-surface"`, `/` gateway panel renders `class="gateway-panel glass-surface glass-surface--dark"`.
- Decisions / assumptions: clamp (not fail-build) per task-spec owner choice; worst-case ratio = min(vs-white, vs-canvas) so both text-on-fill and fill-on-canvas uses are covered; allowlist order is preference order and first-pass wins (#087c73 always reachable as fallback default when nothing passes — unreachable today since #087c73 passes); token value alignment for `--glass-blur-*`/`--glass-saturate` is output-neutral because no consumer existed prior to the utility.
- Deferred or risk IDs: RISK-0014 CLOSED (2026-08-23); DEBT-0012/DEBT-0013 CLOSED (2026-08-23); none opened.
- Rollback / recovery: revert the single commit — Header/Gateway regain local glass blocks, BaseLayout regains unvalidated injection; no schema/data impact.

## LOG-0240 — 2026-08-23 — B5 Candidate 1: CSS-only hero constellation entrance (ADR-0030)

- Outcome: One-time stroke-draw entrance for the locale-landing constellation SVG (`/en/`, `/fa/`) shipped as **CSS-only — 0 bytes of new JS**. `pathLength="1"` + `stroke-dasharray:1; stroke-dashoffset:1` draw animation on spokes/ticks/center-ring (1.2s, `var(--ease-out)`, staggered delays 100–300ms spokes, 550–760ms ticks); dashed connectors fade to their static `opacity:.55` (they keep their `4 7` dash look, so they cannot use the dash-draw trick); nodes/core scale+fade in after paths (700–1100ms). Entire block wrapped in `@media (prefers-reduced-motion: no-preference)`; base styles are final-state so reduced-motion and no-JS users get the identical static SVG. Hero copy has no animation. Gateway `/`, Three.js work and other pages untouched by this entry.
...
- Decisions / assumptions: **B5 Candidate 3 (Journey timeline reveal / gsap) SKIPPED per instruction chain** — Landing.astro has no journey/timeline markup and `content.ts` has no journey data (only the About-page `sections.experience` label; Profile experience feeds About/CV, not a landing Journey §71 section which is not implemented). Creating timeline content or a new homepage section would invent content/IA beyond this motion task; escalate when a Journey section is designed/authorized as a separate Task Spec. Note: a concurrent session added `<Constellation3D client:visible />` (Three.js Candidate 5) to Landing.astro during this task; left untouched per scope (dist shows `Constellation3D` 2.26 KB gzip + lazy `three.module` 125.64 KB gzip chunks, within the ≤150 KB budget).
...
## LOG-0239 — 2026-08-23 — Deploy LOG-0238: دو فیل CD رفع و سبز شد — P9 routes live

- Outcome: پوش LOG-0238 دو فیل CI/CD داد که هر دو رفع و re-deploy شد. **فیل 1 (run 32665496):** Admin frontend CI + Web image + CMS CI + CD + CMS image همگی fail — (الف) SPA `entities.ts:20` `"course"` در `ContentEntity` union نبود (`api.ts:33`) → TS2322؛ fix: union +`"course"|"creative-work"`، SPA build PASS. (ب) Web image build با `CMS_API_BASE=https://tahamohamadi.ir` → `creative-works/en page 1: unexpected HTTP 404` — وب جدید endpointهای P9 را می‌خواهد ولی CMS پروداکشن (`705ed4c`) هنوز `content.0015` ندارد → fail-build صحیحِ B1 پنجره web-ahead-of-CMS را بست؛ fix: bridge موقت 404→honest-empty در `creative.ts:57`/`courses.ts:48` با کامنت «FLIP TO THROW after content.0015 migrate» (همان الگوی P8 قدیم) + Task-list flip item. **فیل 2 (run 32666593983):** CD در `Reload Compose Caddy` — compose حالا `${WEB_IMAGE:?}` fail-closed است و reload script بدون env اجرا می‌شد؛ fix: `caddy-compose-reload.sh:35` CMS_IMAGE/WEB_IMAGE را از کانتینرهای درحال‌اجرا derive می‌کند (reload تصویر اپ را عوض نمی‌کند). **Deploy موفق (run 32667157006):** Build static 2m50s + Deploy 58s PASS؛ annotation «No web image for 5ba7d4e; using fallback :main (CADDY_EDGE=compose avoids stale web)» = فیکس Infra#2 LOG-0237 در عمل کار کرد. **Live verify:** `/en/teaching/`+`/fa/teaching/`+`/en/creative/`+`/fa/creative/` هر دو locale 200 (honest-empty)، `projects/publications/writing/contact` سالم، `health.json ok`، 4/4 container healthy، web recreated با `:main` جدید (شامل P9 routes). CI/CMS CI/Admin CI/CMS image/Web image همگی سبز روی `3ce3fa9`.
- Why: deploy سبز و بدون stale شرط پایان اسپرینت بود؛ هر فیل یک درس معماری داد (union typing، web-ahead-of-CMS، fail-closed env derivation).
- Scope / files: `apps/cms/admin-frontend/src/lib/api.ts` (union)، `apps/web/src/lib/cms/{creative,courses}.ts` (bridge)، `infra/deploy/caddy-compose-reload.sh` (derive)، این entry. Commits: `3ce3fa9` + `5ba7d4e`.
- Commands or actions actually performed: `gh run view --log-failed` ×2، TS union fix + SPA `npm run build` PASS، 404-bridge + build با `CMS_API_BASE` (96p PASS) و بدون (58p PASS)، reload script derive + `bash -n`، push ×2، `gh run list/view` تا سبز، `Invoke-WebRequest` ×8 routes + `ssh docker ps/inspect`.
- Verification actually performed and result: هر 6 workflow سبز روی آخرین push؛ CD PASS با fallback annotation؛ live routes 200؛ containers healthy.
- Deferred or risk IDs: **`content.0015/0016` prod migrate هنوز owner-attended (RISK-0012)** — تا قبل از آن bridge 404 فعال است؛ flip-to-throw بعد از migrate (Task-list P9 section). Infra#9 timers OPEN. KI-0007 OPEN.
- Rollback / recovery: `git revert 5ba7d4e 3ce3fa9`؛ bridgeها با حذف `if 404` بسته می‌شوند؛ reload script derive بی‌ضرر است.

## LOG-0238 — 2026-08-23 — P2 latent بسته شد + P9/P10/P11 scaffold — 5 commit گروهی با گیت مدیریت‌شده

- Outcome: ادامه اسپرینت با 3 agent موازی (B7/B8، Infra#3/4/5، Tests) + 2 agent P9/P10/P11 (rate-limit شدند، ادامه دستی). **P2 بسته شد:** B7 `siteSettings.ts:42` `getPublicSiteSettings` 404 وقتی `isCmsOriginBuild()` → `CmsOriginError` (mock verify: fail با base، null بدون)، B8 `research-graph.ts:86` `topicDetailsRaw`/`projectDetailsRaw` با `missingTopics.length>0` throw (node drop ممنوع در CMS build)، Infra#3 `docker-compose.cms.yml:50,88` `${CMS_IMAGE:?}`/`${WEB_IMAGE:?}` fail-closed + `pull_policy: always` (env override برای CMS_BUILD=1→never)، #4 `Dockerfile.web:15` `nginx:1.27-alpine@sha256:65645c7b...` digest-pinned، #5 mount `create_host_path: false` + `caddy-compose-reload.sh:27` guard `mkdir -p /var/www/html`، Tests: `qa/pagefind.spec.mjs` (per-locale entry/fragment/page_count + draft/admin exclusion)، `qa/sitemap.spec.mjs` (static mirror + dynamic detail walk + lastmod + canonical publications + disallowed paths)، `qa/health.spec.mjs` (static vs CMS health distinction) + `smoke.sh:47` `"service":"static"` + `/health/` 404-static/200-CMS + `ci.yml:102` سه step جدید. **P9 scaffold:** `content.0015_p9_courses_creativeworks` — Course (level/prerequisites/outcomes/format/language/availability/license/last_updated) + CreativeWork (creator/role/date/license/access_state/rights_statement/consent_verified، بدون student PII)؛ admin schema `course`/`creative-work` (`admin_content.py:69`)؛ public projection + `lib/cms/courses.ts`/`creative.ts` + routes `/teaching/` (canonical list+detail) و `/creative/` (list+detail) fa+en honest-empty؛ duplicate `/courses/` routes حذف شد (IA تمیز — teaching canonical). **P10/P11:** `content.0016_collection_and_taxonomy_glossary` (Collection: curator/criteria/date curation contract) + `docs/governance/TAXONOMY_GOVERNANCE.md` + ADR-0029 draft `docs/adr/0029-p10-p11-ai-gate.md` (threat model، owner-gated، no private ingest). **Fixes حین verify:** ruff E501×2 در models.py، `test_schema_endpoint` entities +course/creative-work، sitemap spec staticPaths +teaching/creative (بعد از حذف courses). **Verification نهایی:** ruff clean، pytest **393 passed**، `npm run check` 0 err (139 files)، build 58 pages، هر 4 spec PASS (p8/pagefind/sitemap/health)، `docker compose config` fail-closed verify. 5 commit: `171c455` (B7/B8) → `fef3178` (Infra) → `9441ac0` (tests) → `5459256` (P9) → `2f404a8` (P10/P11).
- Why: درخواست مالک «P2 latent + P9/P10/P11 را دقیق و کامل انجام بده، در هر مرحله گیت را مدیریت کن».
- Scope / files: `apps/web/src/lib/cms/{siteSettings,research-graph}.ts`، `infra/cms/docker-compose.cms.yml`، `infra/web/Dockerfile.web`، `infra/deploy/{caddy-compose-reload,smoke}.sh`، `apps/web/qa/{pagefind,sitemap,health}.spec.mjs`، `.github/workflows/ci.yml`، `apps/cms/apps/content/models.py` + `migrations/0015,0016`، `apps/cms/apps/api/{api,admin_content}.py`، `apps/cms/admin-frontend/src/lib/entities.ts`، `apps/web/src/lib/cms/{courses,creative}.ts`، `apps/web/src/pages/{en,fa}/{teaching,creative}/`، `apps/web/src/data/content.ts`، `apps/web/src/pages/sitemap.xml.ts`، `docs/governance/TAXONOMY_GOVERNANCE.md`، `docs/adr/0029-p10-p11-ai-gate.md`، `Task-list.md`، این entry.
- Commands or actions actually performed: `Task ×3` موازی (B7B8/Infra/Tests موفق، P9/P10P11 rate-limit → ادامه دستی)، `uv run ruff check --fix` + 2 edit دستی E501، `uv run pytest -q` (1 fail schema → fix entities → 393 pass)، `npm run check/build`، `node qa/*.spec.mjs` ×4، `docker compose config` با/بدون env، `git add` گروهی ×5 + commit ×5.
- Verification actually performed and result: همه suites سبز؛ fail-closed compose verify (بدون env → error)؛ specs PASS؛ build 58p شامل `/teaching/`+`/creative/` هر دو locale.
- Deferred or risk IDs: P10-03 FTS + P10-04 lifecycle deferred (content-volume gated)؛ P9-03 keyboard QA deferred؛ **`content.0015/0016` prod migrate = owner attended dispatch (RISK-0012 path)** — بدون آن CMS جدید بالا نمی‌آید مگر migrate؛ Infra#9 timers هنوز OPEN.
- Rollback / recovery: `git revert 2f404a8..171c455`؛ migrationها additive و reversible؛ compose `:?` در صورت نیاز به `:-` برگردانده شود.

## LOG-0237 — 2026-08-23 — اجرای P0/P1: fail-closed P8/contact، CMS-driven home/footer، حذف CV md، cms-build-origin و CI/CD infra (موازی)

- Outcome: تمام موارد P0/P1 درخواستی در دو گروه موازی اجرا شد. **گروه A (web):** B1 `publications.ts:133` 404→throw (حذف `if 404 return null`, حالا مثل `articles.ts:69` fail-build؛ `sitemap.xml.ts:99` هم fail)، B2 `siteSettings.ts:56` `getSiteContact():null` فقط وقتی `isCmsOriginBuild()` false از snapshot می‌گیرد و `Footer.astro:21`/`ContactPage.astro:10` honest-empty، B3 `Landing.astro:3` → `Props {profile}` + `lib/cms/landing.ts:1` `loadLandingProfile()` (unset→snapshot, set→`GET /api/profiles/...` با throw)، `pages/{en,fa}/index.astro:1` آن را پاس می‌دهد، B4 `content.ts:219` → `siteSettings.ts` `getSiteBrand/Tagline/Footer/Seo` + `Footer.astro:31`/`Header.astro:1`/`BaseLayout.astro:9` با `effectiveTitle/Description` و `cms-build-origin` همیشه، B5 `public/downloads/*.md` (2 فایل 298 خط) حذف و به `fixtures/downloads/` منتقل (dist دیگر 0 md), B6 هر 7 کاتالوگ (`writing/research/projects/publications/books/talks/downloads` + `en/fa/index`) اکنون `BaseLayout cmsBuildOrigin={isCms?"cms":"snapshot"}` دارند. **گروه B (infra):** Infra#1 `ci-cms-image.yml:14` +`admin-frontend/**`, Infra#2 `cd.yml:186` fallback به `:main` با `WEB_PULL_POLICY=always` برای جلوگیری از stale روی docs-only push, CI `ci.yml:44` + `p8-catalog.spec.mjs:1` (mock 127.0.0.1:0 با `spawn` async, locale-aware, pagination) + job `p8-catalog-mock:122`, Infra#8 `health/views.py:5` `degraded` وقتی contact enabled اما `EMAIL_HOST` خالی + `.env.example:17` doc. Verification: `npm run check: 0 err 9 hints (122 files)`, `npm run build` snapshot 54p و mock CMS 0→3 pubs PASS, `node qa/p8-catalog.spec.mjs: PASS` + restore, `pytest: 393 passed`, `py_compile health/views.py` degraded/ok, `yaml.safe_load` 3 workflows OK.
- Why: همه P0 fail-closed باید همین اسپرینت بسته شوند تا استقرار بعدی دوباره 0/3 ندهد.
- Scope / files: `.github/workflows/cd.yml,ci-cms-image.yml,ci.yml`, `apps/cms/apps/api/api.py,apps/health/views.py`, `apps/web/src/lib/cms/{publications,siteSettings,landing}.ts`, `components/{Landing,Footer,Header,ContactPage,Downloads}.astro`, `layouts/BaseLayout.astro`, `pages/{en,fa}/{index,writing,research,projects,publications,books,talks,downloads}/index.astro`, `data/cvDownloads.ts`, `infra/cms/.env.example`, `public/downloads/*.md` D + `fixtures/downloads/*.md` A + `qa/p8-catalog.spec.mjs` + `lib/cms/landing.ts`, این entry. `dist` ignored.
- Commands or actions actually performed: `Task ×2` parallel, `npm run check/build` (snapshot + CMS 92p), `node qa/p8-catalog.spec.mjs`, `uv run pytest -q`, `python -m py_compile`, `git status/diff`.
- Verification actually performed and result: B1 mock 404 → CmsOriginError PASS, B2 snapshot only when unset PASS, B3/B4 CMS title/tagline 0→1 humanCount PASS, B5 md in dist false PASS, B6 cms-origin meta on 7 pages PASS, p8 parity 3/3 PASS, health degraded/ok PASS, workflows YAML OK.
- Deferred or risk IDs: B1/B2/C3 P0 بسته شد؛ KI-0007 OPEN؛ DEBT-0008 P2 هنوز (design docs)؛ B3-B6 P1 بسته شد.
- Rollback / recovery: `git revert HEAD` یا `git checkout HEAD~1 -- apps/web/src/lib/cms`؛ fixtures را دوباره به public برگردانید.

## LOG-0236 — 2026-08-23 — Audit کامل bug و به‌روزرسانی Task-list — 15 باگ پنهانِ deployment/content + 3 بهبود Task-list

- Outcome: به‌درخواست «کامب بررسی کن باگ مخصوصا اینحوری نداشته باشه + تسک‌لیست به‌روز»، ممیزی سه‌جانبه موازی (web projection، infra، tests) با 3 sub-agent اجرا شد. **یافته‌های وب (9):** B1 P8 404→`[]` در `publications.ts:133` (بعد P8 live باید fail-build شود مثل `articles.ts:69`), B2 `siteSettings.ts:56` snapshot fallback با وجود `CMS_API_BASE` (خلاف ADR-0027), B3 home همیشه static `Landing.astro:3` (A1), B4 footer/tagline static `content.ts:219` (A3), B5 CV md هنوز سرو `public/downloads/*.md` (A4), B6 `cms-build-origin` فقط روی cv/about (smoke نمی‌تواند stale را بگیرد), B7-9 latent. **یافته‌های infra (13):** #1 `ci-cms-image.yml:14` فاقد `admin-frontend/**` → SPA stale، #2 `cd.yml:195` dual-path (host symlink vs web:8080) روی docs-only push استیل، #3 `pull_policy:missing` + `:main` mutable، #4 `nginx:alpine` unpinned، #8 `EMAIL_HOST=""` → 503 honest اما بدون boot validation، #9 timers not enabled، #10-12 CORS/rate-limit/CSP latent. **یافته‌های تست:** P0 `ci.yml:36` vs `cd.yml:55` (CI بدون CMS_API_BASE) + نبود `API↔HTML parity spec` برای هر 4 P8 کاتالوگ + `cms-origin-honesty` در CI اجرا نمی‌شود → stale قبلی را نمی‌گرفت. **اقدام:** `Task-list.md:38` Production snapshot به `705ed4c/e782` به‌روز شد، Hotfix جدید اضافه، بخش جدید `Audit — deployment/content gaps` با 6×P0 + 6×P1 + P2 افزوده شد (هر مورد با `file:line`). ADM-0 تیک‌های `DEBT-0003` (dumpdata + Wagtail uninstall) از `[ ]` به `[x]` با LOG-0193/0216 تصحیح شد.
- Why: حتی یک باگ پنهانِ silent-empty مثل Publications 0/3 هم باید قبل از live بعدی بسته شود.
- Scope / files: `Task-list.md` (46 خط: Hotfix+Audit+snapshot+ADM-0)، این entry. هیچ کد production تغییر نکرد — فقط مستندسازی برای فاز بعد.
- Commands or actions actually performed: `Task tool ×3` parallel audits (read `apps/web/src/lib/cms/*.ts`, `pages/**/*`, `infra/**/*.yml`, `workflows/*.yml`, `tests/**`, `qa/**`), `rg TODO|FIXME` 0, `npm run check` 0, `uv run pytest -q` 393, `git diff --stat` 39 insertions.
- Verification actually performed and result: هر audit فهرست دقیق `file:line` تحویل داد؛ `Task-list` با `git diff` 39+7 خط به‌روز و با `LOG-0215..0235` هم‌تراز شد.
- Deferred or risk IDs: `KI-0007` OPEN؛ `DEBT-0008` OPEN؛ 15 مورد جدید به‌صورت `[ ]` در Task-list ثبت شد تا در اسپرینت بعدی با Task Spec بسته شوند — هیچ موردی gloss نشد.
- Rollback / recovery: `git checkout HEAD~1 -- Task-list.md` یا `git revert`.

## LOG-0235 — 2026-08-23 — ممیزی جامع استقرار: web+cms stale، Caddy، کاتالوگ‌ها و شکاف CI/CD — رفع کامل و پیشنهادات

- Outcome: به‌دنبال تذکر «خیلی مسخره است بین این همه کار مشکل استقرار داشتیم»، ممیزی 5-محوره انجام و دو drift دیگر (علاوه بر web قبلی) رفع شد. **1) تطابق ایمیج:** `taha-cms-web-1:main` قبلاً fix شده به `3ff7bf0f...` (15:27) اما `taha-cms-cms-1:e2cd1b6` (2026-08-22 07:39) هنوز قدیمی بود — `/api/site` فقط `{"primaryColor","downloads"}` و `/api/contact` 404 می‌داد (APIs projects/research درست بودند اما site/contact نه)؛ `showmigrations siteconfig: [X] 0001,0002` بدون 0003. Fix: `ssh deploy@... "CMS_IMAGE=ghcr.io/taha-cms:705ed4c bash infra/deploy/cd-cms-migrate.sh"` → backup 382916B به `cms-migrate-backups/pre-migrate-20260823T153419Z`, `Applying siteconfig.0003_sitesettings_contact... OK`, smoke-cms.sh 9/9 PASS, `CMS_IMAGE` در `infra/cms/.env` به `705ed4c` persist شد. اکنون `/api/site` → `{"contact":{"email":"taha.mohammadi@...","formEnabled":true}}` بدون phone (درست)، `/api/contact` GET 405 و POST 503 honest (SMTP not configured, per LOG-0230). **2) Caddy:** `ss -tlnp :80/443` → Docker `taha-cms-caddy-1` (Compose edge) و `systemctl is-active caddy: inactive` — درست per DEFER-0031 (Host file در `/etc/caddy/Caddyfile` فقط rollback). **3) کاتالوگ‌ها:** `Invoke-WebRequest /api/publications/en?page=1: count 3` (dashboard-design, visual-discourse, vtd-edge) اما `/en/publications/` هنوز `No published` سرو می‌کرد — چون GHCR `:main` قبلی (2e95...) با cache `gha` بدون `--no-cache` ساخته شده و 0 داشت. Fix: `docker build -f infra/web/Dockerfile.web --build-arg CMS_API_BASE=https://tahamohamadi.ir -t ghcr.io/taha-web:main . --no-cache` روی VPS (e782..., 2026-08-23 17:21) → `docker run --rm :main` اکنون `dashboard-design`×1 `visual...`×1 `vtd-edge`×1 (3 `<li>`), `docker compose up -d web` → running `e782...` 17:23, live `Invoke-WebRequest /en/publications/?v=...` → `Guidelines True empty False len 9631` (قبل 0/1). `books:0/empty True` و `talks/downloads:0` honest (API هم 0) — درست. **4) CI/CD gap:** `ci-web-image.yml:9` فقط `apps/web/**` را trigger می‌کند — push docs-only مثل `b3607ff` وب را نمی‌سازد؛ و `docker-compose pull_policy: missing` باعث ماندن `:main` قدیمی شد. پیشنهاد C3: یا `workflow_dispatch` دستی پس از publish دسته‌ای، یا افزودن `schedule` nightly rebuild، یا تغییر `WEB_PULL_POLICY=always` در `update-cms.sh`/`cd-cms-migrate.sh` و یا گام `docker compose pull web` در `cd.yml`.
- Why: استقرار باید بی‌درنگ و بی‌نقص پس از هر publish باشد — حتی یک کاتالوگ جاافتاده هم نقض قید مالک است.
- Scope / files: عملیات VPS: `docker pull/build` + `cd-cms-migrate.sh` + `sed -i CMS_IMAGE` در `infra/cms/.env`, `docker compose up -d web`, این entry. کد وب قبلاً درست بود — فقط artifactها stale بودند.
- Commands or actions actually performed: `docker ps --format`, `docker inspect`, `docker images`, `Invoke-WebRequest /api/site|/api/contact|/api/publications?page=1|live HTML` قبل و بعد، `showmigrations`, `cd-cms-migrate.sh` با backup, `docker build --build-arg CMS_API_BASE --no-cache`, `docker exec ... grep -c`, `Invoke-WebRequest` post-fix برای هر 7 مسیر, `ss -tlnp`, `systemctl is-active`, `cat infra/cms/.env`.
...
- Deferred or risk IDs: `KI-0007` OPEN؛ هیچ ریسک جدیدی باز نشد. `RISK-0012` path با `cd-cms-migrate.sh` دوباره اثبات شد.
- Rollback / recovery: `CMS_IMAGE=e2cd1b6 && bash update-cms.sh` (backup موجود), `docker tag ghcr.io/taha-web:3ff7... :main && up -d`.

## LOG-0234 — 2026-08-23 — Fix: web container stale (Aug 19) → pull ghcr.io/taha-web:705ed4c + recreate (Projects/Writing/Research live)

- Outcome: اسکرین‌شات‌های دوم کاربر (Projects و Writing هر دو `No published ... in this build` + پنل ادمین با 6 پروژه/4 مقاله منتشرشده) نشان داد با وجود API پر (`/api/projects/en` و `/api/articles/en` هر دو 200 با PARS-SQL) سایت استاتیک همچنان خالی سرو می‌شد. ریشه یابی دقیق: **Host file** `/opt/taha/site/current/en/projects/index.html` (release-d6a9731) درست دارای 3 پروژه (`PARS-SQL` 1, `No published` 0) و `en/writing` دارای 2 مقاله بود — اما **Caddy Host** طبق `infra/caddy/Caddyfile:40` `handle /static` نه، `taha_application_routes → reverse_proxy 127.0.0.1:13080` یعنی **taha-cms-web-1 (nginx)** را سرو می‌کند، نه host file_server. کانتینر `taha-cms-web-1` با `ghcr.io/tahamohamadi-ir/taha-web:main` با ID `6e2eeccd8762c8849...` (Created 2026-08-19T18:12:59) مانده بود؛ در حالی که GHCR تگ‌های `705ed4c:3ff7bf0f...` و `main` جدید (2026-08-23T14:47:27) موجود بود ولی `docker images` روی VPS نشان داد `:main` هنوز به قدیمی اشاره دارد (pull نشده). `docker exec taha-cms-web-1 cat /usr/share/nginx/html/en/projects/index.html` → `PARS 0 / No published 1` و `ls -lh` Aug 19 18:12؛ در مقابل `docker run --rm ghcr.io/taha-web:705ed4c cat ...` → `PARS 1 / No 0` و Aug 23 14:46. Fix: `ssh -i taha-nls1-production -p 2222 deploy@85.192.29.196 "docker pull ghcr.io/tahamohamadi-ir/taha-web:main"` → Downloaded newer image `sha256:3ff7bf0f...`; سپس `cd /home/deploy/cms-repo && docker compose -f infra/cms/docker-compose.cms.yml up -d web` → Recreated `taha-cms-web-1` با ID جدید `3ff7bf0f...` Created 2026-08-23T15:27:45. بلافاصله `docker exec` → projects `PARS 1/0`, writing `PARS 1`, research `PARS 2`; live HTTP `Invoke-WebRequest https://tahamohamadi.ir/en/projects/?v=...` → `PARS True empty False len 14791`, writing/research/contact همگی True؛ `bash infra/deploy/smoke.sh https://tahamohamadi.ir` 7/7 PASS؛ `docker exec wget` داخل کانتینر همگی 1. قبلاً rebuild لوکال با `CMS_API_BASE=https://tahamohamadi.ir npm run build` 80 صفحه (vs offline 52) صحت CMS wiring را ثابت کرده بود — مشکل فقط استقرار کانتینر بود، نه کد. Gap مستند شد: `ci-web-image.yml:62` با `CMS_API_BASE` درست می‌سازد اما `cd.yml` برای push docs-only (paths `apps/web/**` ندارد) وب را نکشید؛ `compose pull_policy: missing` باعث ماندن `:main` قدیمی شد.
- Why: چیزی نباید جا بماند — محتوای منتشرشده باید بی‌درنگ روی `tahamohamadi.ir` دیده شود.
- Scope / files: هیچ فایل کدی تغییر نکرد (artifact وب قبلاً درست بود 705ed4c) — فقط عملیات VPS: `docker pull` + `docker compose up -d web` (compose `taha-cms` در `/home/deploy/cms-repo/infra/cms/docker-compose.cms.yml:87`). این entry.
- Commands or actions actually performed: `ssh deploy@... docker images --format`, `docker inspect taha-cms-web-1`, `docker run --rm :705ed4c cat ... | grep -c`, `Invoke-WebRequest live api + live html` (2 دوره قبل و بعد فیکس), `CMS_API_BASE=... npm run build` 80p, `docker pull :main`, `docker compose up -d web`, `docker exec ... grep -c` post-fix, `Invoke-WebRequest ... projects/writing/research/contact` post-fix, `bash smoke.sh`.
- Verification actually performed and result: pre-fix file mismatch ثابت شد (host 1 vs container 0); post-fix هر سه کاتالوگ `PARS True empty False` زنده؛ smoke PASS؛ `docker ps` 4/4 healthy.
- Deferred or risk IDs: `KI-0007` OPEN؛ `DEBT-0008` OPEN؛ ریسک جدید `CLOSED` شد — از این پس هر deploy که وب را تغییر نداده ولی `:main` عقب است، نیاز به `pull` دارد. پیشنهاد: `cd.yml` را به `WEB_PULL_POLICY=always` یا گام `docker compose pull web` پس از push تگ main تجهیز شود (Task-list C3).
- Rollback / recovery: `docker image tag ghcr.io/taha-web:6e2ee... :main` و `compose up -d` به عقب؛ یا `docker run --rm :705ed4c` همیشه سالم است.
