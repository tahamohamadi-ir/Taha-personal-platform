...
- Outcome: Owner-attended Caddy cutover on VPS `deploy@85.192.29.196:2222` (SSH key `taha-nls1-production`). **First attempt:** host Caddy disabled → Compose `caddy` started without seeded ACME data → public TLS **HTTP 525** (Cloudflare origin cert mismatch). **Rollback:** Compose `caddy` stopped → host systemd Caddy re-enabled → `smoke-cms.sh` **PASS**. **Recovery:** copied host `/var/lib/caddy` into Docker volume `taha-cms_caddy_data`; host Caddy disabled again; Compose `caddy` restarted. **Second cutover PASS:** `curl -sI https://tahamohamadi.ir/` → **HTTP/2 200**; `smoke-cms.sh` → **PASS** (all checks). Agent re-attestation (same day): host Caddy **inactive/disabled**; `taha-cms-caddy-1` **Up** on **80/443** (healthcheck `unhealthy` but serving); apt upgradable **3** phased packages (`python3-software-properties`, `snapd`, `software-properties-common`); SSH listens **22+2222** (dual-port accepted); loopback POST `/rebuild-trigger/` bad token → **403**; anonymous `/api/v1/admin/openapi.json` → **404**; `PREVIEW_SHARE_SECRET` length **64**. **`CADDY_EDGE=compose` not set** (GitHub repo variable absent; owner to set via web UI or `gh`). `smoke.sh` → **PARTIAL** (`/nonexistent-qa` returns **200** not 404 — nginx SPA fallback via Compose edge). Live `/en/about/` HTML lacks `<meta name="cms-build-origin">` in current public build (separate from edge cutover). **`DEFER-0031` CLOSED**; **`RISK-0013` CLOSED**; **`RISK-0005` CLOSED** (attended apt reduced pending set to 3 phased-only packages); **`RISK-0006` CLOSED** (canonical deploy **2222**; port **22** retained for VPN — accepted residual). Did **not** set `CMS_CD_AUTO_MIGRATE`.
- Why: Close ADR-0027 Slice 4 live TLS edge after owner recovery from first-cutover 525.
- Scope / files: VPS attestation only (no VPS writes this session); this entry; ledger sync in same PR.
- Commands or actions actually performed: SSH read-only checks (`systemctl`, `docker ps`, `apt list --upgradable`, `ss -lntp`, `curl`, `smoke-cms.sh`, `smoke.sh`, loopback HMAC probe, `printenv PREVIEW_SHARE_SECRET`); GitHub API check for `CADDY_EDGE` variable.
- Verification actually performed and result: Cutover criteria met except **`CADDY_EDGE`** (owner follow-up) and **`smoke.sh` 404 probe** (non-blocking for edge). Compose caddy healthcheck `unhealthy` while public TLS serves **200**.
- Deferred or risk IDs: `DEFER-0031` **CLOSED**; `RISK-0013` **CLOSED**; `RISK-0005` **CLOSED** (3 phased apt remain); `RISK-0006` **CLOSED** (dual-port acceptance); `CADDY_EDGE=compose` **OPEN** (CD gate).
