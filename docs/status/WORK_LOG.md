...
- Outcome: Accepted **ADR-0028** (option B: SVG + MIT `motion`, keep **35KB gzip** island budget; defer gsap proprietary + three.js over-budget). Implemented progressive React island on `/{locale}/research/` with `client:visible`, build-time edges from published topic/project projections, and a complete HTML relationship tree for no-JS. Added `@astrojs/react` + `react`/`react-dom`. Updated B5 Candidate 4, LICENSES motion note, `DEFER-0020` (graph viz partial close; curated collections remain OPEN). Dist QA `qa/research-graph.spec.mjs`. Left uncommitted on `feat/wave4-research-graph`.
- Why: Owner asked for the best interactive research map; licenses/budget constrain to motion+SVG rather than gsap/three without a further ADR.
- Scope / files: `docs/adr/0028-research-graph-interactive-island.md`, `docs/adr/README.md`, `docs/plan/wave4-research-graph-island-task-spec.md`, B5/LICENSES/plan README, deferred-validation/BACKLOG, `apps/web` research graph components + React integration + content chrome strings + QA spec; this entry.
- Commands or actions actually performed: worktree `feat/wave4-research-graph` from `origin/main`; `npm install` (+ react / `@astrojs/react`); `npm run check` (0 errors); `npm run build` (offline + `CMS_API_BASE=https://tahamohamadi.ir`); `node qa/research-graph.spec.mjs`.
- Verification actually performed and result: `astro check` **PASS** (0 errors/warnings); offline build **PASS**; CMS-origin build **PASS** (68 pages; graph + tree present on `/en/research/` and `/fa/research/`); QA **PASS**; island chunk `ResearchGraphIsland.*.js` ≈ **23838 bytes gzip** (&lt; 35KB ADR-0028 budget); no `gsap`/`three` imports under `apps/web/src`.
- Deferred or risk IDs: `DEFER-0020` **OPEN (collections)** — interactive graph shipped; gsap/three still gated.
