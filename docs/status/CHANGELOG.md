...
- Admin API `POST /api/v1/admin/content/{entity}/{id}/preview-link` + SPA ???? ???? ?????????? on edit pages.
...
- OWNER_CUTOVER step 6 **CLOSED** (manual path). Required gates complete; optional `DEFER-0027` / `DEFER-0031` remain OPEN. Never `CMS_CD_AUTO_MIGRATE`. LOG-0201. **GOAL_COMPLETE=yes**.

## 2026-08-22 ? CD timer NOPASSWD wrapper (fix 32556305961)

- First `install_scheduled_timer=true` dispatch [32556305961](https://github.com/tahamohamadi-ir/Taha-personal-platform/actions/runs/32556305961) **FAILED**: deploy lacks NOPASSWD for repo-path `install-scheduled-publish-timer.sh`.
- Added root-owned wrapper `/opt/taha/bin/install-scheduled-publish-timer.sh` + owner one-time `install-scheduled-publish-timer-sudo.sh`; CD uses `sudo -n /opt/taha/bin/install-scheduled-publish-timer.sh`.
- Owner VPS prereq before re-dispatch: `sudo bash infra/deploy/install-scheduled-publish-timer-sudo.sh`. Never `CMS_CD_AUTO_MIGRATE`. LOG-0200.
...
- `infra/caddy/Caddyfile.compose`: `/admin-wagtail/` ? `/staff/*`. LOG-0194.
...
- **Backend** ?? ???? `apps/cms/apps/composition/`: ??????? `CompositionPage` (key ????? ????? locale fa/en? title? status draft/review/published/archived? published_at? created/updated_at)? `CompositionSection` (page FK? position? layout 1col/2col/3col? ratio? enabled? UniqueConstraint page+position)? `CompositionBlock` (section FK? position? block_type? settings JSONField? enabled? UniqueConstraint section+position)? migration `0001_initial.py`? `blocks.py` ?? ??????? hero/heading/text/quote/cta/gallery/divider + `validate_block_settings` fail-closed + `SECTION_LAYOUT_RATIOS` + `composition_schema()`.
- **API** `apps/api/admin_composition.py` mount ?? `/api/v1/admin/composition`: GET ????? (q/locale/status/page/pageSize? 400 VALIDATION)? POST create (201? key regex `^[a-z0-9-]+$`? 409 DUPLICATE)? GET /schema? GET /{id}? PUT /{id} ???????? full-document (If-Match + select_for_update + atomic? 409 CONFLICT ?? currentUpdatedAt? fail-closed ?? field paths ??? `sections[0].blocks[1].settings`). Guards: staff+OTP+CSRF? ????? ????? strict int (float/bool ??).
- **Frontend** `apps/cms/admin-frontend/`: `src/lib/api.ts` (Composition types + fetchCompositionPages/Schema/Detail + createComposition/updateComposition)? `src/lib/composition.ts` (labels? ratioOptionsFor? REQUIRED_BLOCK_FIELDS)? `src/pages/CompositionListPage.tsx`? `src/pages/CompositionEditorPage.tsx` (schema-driven: layout/ratio ???????? ??????? ?? ??????? text/textarea/select/media/mediaList ?? ???? MediaPicker? ????????? grid? ?????????? client ???? ??????? ??????? 409 reload/discard? dirty-guard)? routes /composition? ??????? ???????.
- **??????????:** 249 passed (20 ??? `test_admin_composition_api.py` ?? regression ????? ????? float/bool)? ruff clean? ???? migration ????? SPA build/check PASS.
- **????:** projection ????? (rendering ?? Astro) ? ADM-6? rich blocks v2 (?14 U3) ????? Caddy no-store ? deploy ????? ???? CMS ??????? ????? DEFER-0023 ???? ?????? RISK-0010 ???? ?????.

## 2026-08-18 ? ADM-2: media library (upload/replace, alt-by-locale, orphans)

- **Backend** `apps/cms/apps/api/admin_media.py` (new): `/api/v1/admin/media` ? GET list (q / type image|pdf / active true|false / page/pageSize? 400 VALIDATION)? POST multipart upload (201? `is_active` ??????? false? `full_clean` ? 400)? GET /orphans (usage==0)? GET /{id}? PUT /{id} (optimistic lock If-Match ???? atomic+select_for_update? 409 CONFLICT ?? currentUpdatedAt)? POST /{id}/replace (???????????? MIME? 400 ?? ?????????????/?????). `media_usage_count` + `MEDIA_REFERENCE_FIELDS` (??????? ????? ?? ADM-3 ??? ??????). Guards: staff+OTP+CSRF.
- **Model/migration:** `alt_text_fa`/`alt_text_en` ??? `apps/cms/apps/media/models.py` (CharField blank default "" + `db_default=""`)? migration `0002_media_alt_text_en_media_alt_text_fa.py` (AddField ?? db_default ? ??? ??? ???????? ????? Postgres)? ???? DEFER-0014? makemigrations --check ???? pending.
- **Frontend** `apps/cms/admin-frontend/`: MediaLibraryPage (???????? orphan toggle? ????? ?? progress? drawer ?????? ?? ???????? ???? + ????? ??????? + 409 reload/discard)? MediaPicker (modal ???? reuse)? MediaThumb? `api.ts` (uploadMedia/replaceMedia ?? XHR+progress)? route `/media`? ??????? ????????? ??????.
- **??????????:** 229 passed (20 ??? `test_admin_media_api.py` ?? 8 regression)? ruff clean? ???? migration ????? SPA build/check PASS.
- **????:** ????? MediaPicker ?? ??????????? ????? ?? ADM-3 ????? ?? (DEBT-0004)? Caddy no-store ? deploy ????? ???? CMS ??????? ????? DEFER-0023 ???? ?????? RISK-0010 ???? ?????.

## 2026-08-18 ? ADM-1: content write API (create/update + optimistic lock) + SPA edit pages

- `/api/v1/admin/content/*` ???? write ?? ????: `POST /{entity}` (create 201? 409 DUPLICATE)? `PUT /{entity}/{id}` (optimistic lock If-Match ???? `select_for_update`? 409 CONFLICT/DUPLICATE)? `GET /schema` (???????? ??????). Guard: staff+OTP+CSRF? publish ??? ???? `published_at` ???? ???? ???? ???? ???? skip? ????? ?? ???? camelCase.
- SPA: ?????? ??????/???? (`/content/:entity/new` ? `/:id/edit`) ?? ??? schema-driven? ?????? ?+ ?????? ?????? 409 ?? reload/discard.
- ??? ??? ??? (?? ??? write + ? regression)? ruff ????? ???? migration? review ????? ?? ? ???.

## 2026-08-18 ? ADM-1: content read API + dev preview route + SPA content pages

- **Content read API:** `GET /content/{entity}` (????? ?? ???????? locale/status/q + ????????? page/pageSize? ?????? 400 VALIDATION / 404 NOT_FOUND) ? `GET /content/{entity}/{id}` (?????? ?? ?? camelCase `fields`) ???? ? ???????: landing, profile, article, research-topic, research-statement, project, publication ? ?? `apps/cms/apps/api/admin_content.py` (new) ? mount ?? `/api/v1/admin/content/`. `apps/cms/apps/api/admin_common.py` (new): AdminError? error handler? CSRF check? staff/OTP guards ? client_ip ????? ????.
- **Dev preview route:** `serve_admin_ui` ?? `/admin-ui/` ? DEBUG-only? path-traversal-safe (resolve+startswith)? SPA fallback ?? index.html? ?????? `X-Robots-Tag: noindex, nofollow, noarchive` + `Cache-Control: no-store` (`apps/cms/apps/api/admin_spa.py` (new)? mount ?? `config/urls.py`).
- **SPA:** `apps/cms/admin-frontend/` ? ContentListPage (?????? entity? ???????? locale/status/q ?????? ?? URL? ?????? debounced? ???? RTL? ?????????? ???????? loading/empty/error) ? ContentDetailPage (???? generic `fields`? ????????? dir=ltr? ???? 404)? `src/lib/entities.ts` + `src/lib/format.ts` ????? `src/lib/api.ts` ?? fetchContentList/fetchContentDetail + types? ??????? ??????? ?????? ? /content? `vite.config.ts` base `/admin-ui/` ? BrowserRouter basename `/admin-ui/`.
- **??????????:** backend 195 passed (7 ??? ???? `test_admin_content_api.py` + 14 ??? auth)? ruff clean? ???? migration ????? SPA build/check PASS? smoke ????????? dev PASS (deep-route fallback 200? traversal 404? DEBUG=False 404? missing build 404 ?? hint).
- **????:** write/update + optimistic locking? cutover ????????SPA ??? `/admin/` (DEFER-0023)? ????? Caddy no-store snippet ??? ???? (??????? ????)? RISK-0010 ???? ?????.

## 2026-08-18 ? ADM-1 foundation: custom admin auth API + React SPA scaffold

- **Backend:** `/api/v1/admin/` (Django Ninja) ? `auth/csrf|login|logout|me` + `dashboard/summary`? session+CSRF ???? + TOTP/recovery + AuditLog + rate-limit? ?????? `{code,message,fields}`? ?? ??? ????? ?? ?????? ??? ???.
- **Frontend:** `apps/cms/admin-frontend/` ? React 18 + Vite + TS + Tailwind v4 + Vazirmatn (RTL ?????)? ????? AuthGuard? ????? ? ???????? build/type-check ?? CI ???? `ci-admin-frontend.yml`.
- **Caddy:** ???? `no-store` ???? `/api/v1/admin/*` ? `/api/admin/*` ?? snippet (????? ??? ???? ???).
- **Additive:** ??????? ? `/admin/` ???? ?? cutover (ADM-1 ?????) ??????????????.

## 2026-08-18 ? Server sync progress + stale CMS image pin fix

- `prod-cms-update-migrate.sh` ??????? ????? `b369885` ?? ?? ??? ???? `CMS_IMAGE` ???? ?????? ??? (????? sha ?? workflow ?CMS image?). ??????? ????? ???? ???? migrations 0005/0006 ????? ????? ? `import_profile_seed` ?? ????? ?????.
- ???? ???? ?? ????? ???: backup ????? ??? ??????? ?????????? = deploy ????? `430061b` + migrate + `import_profile_seed` + ??????? ??????? (VPS ??? ????? ? ?? ??? Node 24 ?? build ???? ?? SSH tunnel).

## 2026-08-18 ? Custom admin rebuild authorized (ADR-0026)

- **????? ????:** Wagtail ?? runtime ? ????? ??? ??????? ??????? = ????? ??????? React SPA ??? `/admin/` + Django Ninja `/api/v1/admin/*` ?? ??? baseline ?????? (session+CSRF+TOTP+audit+rate-limit). ????????? Wagtail-session ????? (`/admin/profiles/` PR #31 ? site content admin PR #24) ?? ADM-1 ?? SPA ????? ???????? ????? ????? Astro ??????? ?? rebuild-trigger ? `/api/`/`/media/` ????? published-only ???? ?????.
- **??? ????? ??????:** ?????? ????? `origin/main` ??? (seed data + ??????? P4?P6 ???? ?????)? `dumpdata` + backup ???? ??? ?? ?? migration? ????/?????/locale ????? ????? ???????.
- **???????:** ADR-0026? ??? ?17 (ADM-0..ADM-6) ?? Task-list? `docs/plan/custom-admin-rebuild-fa.md`? ??????????? Manifest/AGENTS/ledgers? DEFER-0023 (?????? admin)? DEFER-0024 (???? ????)? DEFER-0025 (dark mode)? RISK-0010/RISK-0011? DEBT-0003 ??? ????.
- **???????? ?14** (????? ?? ?????): reading time/JSON-LD/TOC (???? DONE ??? main)? ????? lightbox + ???????? URL-driven (P6)? FTS ????? (P10)? ?????????/OpenAPI/feature flags (ADM)? Vitest + Lighthouse CI + Playwright config + manual-test checklists (QA) ?? Task-list/BACKLOG ????? ????.
- **?????:** ??????? ?? cutover ADM-1 ?? ????? `/admin/` ????? ??????? ?? ?? ??? slice ????? ???? (??? ???????/?????).
...
> ?? slice ????? (??? ???????? ???????) ?? ?? `docs/status/WORK_LOG.md` ??? ?????? ???????? ?? ???? ????? ????? ????? ????. Append-only? ???????? ?? ????.
...
- **Server upgrade (owner):** VPS ????? Ubuntu 26.04 LTS ?? 2 vCPU / ~3910 MB RAM (~4 GiB) / 30 GB disk (~17 GB free) ???? plan 4 GiB ??? ????? ?? (`RISK-0007` CLOSED). stack ???? ?? `docker ps` ?? 2026-08-16 07:19 UTC inventory-confirm ??: `taha-prod-frontend-1` ??? 127.0.0.1:13000? `taha-prod-backend-1` ??? 127.0.0.1:18080? `taha-prod-postgres-1` (`RISK-0004` CLOSED).
- **Staging decommission (ADR-0025):** `staging.tahamohamadi.ir` ?????? ???? ?? ??? ???? ?? (Caddy block ?? VPS ??? ??? DNS record ?? ???? ???? ??? ??? ????? ???? ?? sudo ? ???? deploy user). gate release ????? ??? CI (web + cms workflows) + production smoke ???.
- **KI-0001 CLOSED:** handle ??????? fa ? URL ?????? PARS-SQL ?? `apps/web/src/data/profile.fa.ts` ?? single-m ????? ??? `rg tahamohammadi apps/web/src` ? no matches.
- **CMS gap closure:** NoIndexMiddleware ???? `/admin/`? `/api/`? `/rebuild-trigger/`? JSON logging ????? ?? `production.py` (python-json-logger)? ??????? account-enumeration ? stored-XSS sanitizer ????? ????? **70 pytest PASS** (LOG-0110). CI workflow (owner decision): manage.py test + git diff --check + secret scan steps.
- **Upcoming release:** production ??? release-4fcd19f (checksum `13849ab7`) ???? release ???? ?? HEAD (JSON-LD + KI-0001 fix) ?? ??????? deploy ???.
- **Still blocked:** CMS runtime deploy (MFA enforcement + `RISK-0003` DB-import evidence + deploy Task Spec? `RISK-0009` BLOCKED ? ????? ?? ??? ???).

## 2026-08-15 ? P3 code-first (CMS)

- **P3 gate move (owner-authorized):** `apps/cms/` scaffold + ?? + ??? + CI ???? deploy ?????. Task Spec: `docs/plan/P3-gate-code-first-task-spec.md`.
- **apps/cms:** Django 5.2.9 / Wagtail 7.4.2 / Ninja 1.6.2 / psycopg 3.3.4 ??? Python 3.12.13 (`uv` + `.venv` ??????????? `DEFER-0003` ???? ??).
- **Settings:** split base/development/test/production? production ???? secret?? ?? ?? env ??????? ? ???? `DJANGO_SECRET_KEY`/`ALLOWED_HOSTS`/DB credentials fail ??????.
- **Content:** ??????? Landing/Profile/Article ?? lifecycle (draft/review/published/archived)? `public()` ???????? ???? ???? projection? unicity `(locale, slug)`.
- **API (Ninja):** ??? read-only ?????: list/detail published ?? ???? locale? ???? ????? status/internal fields? 404 ???? stack trace.
- **Media:** ?????????? signature-based ?? `filetype` (jpeg/png/gif/pdf? ?????? 5MB)? ??????? ??? ?????? `is_active` ??????? False (private default).
- **Admin security:** `AuditLog` (???? body/secret)? rate limit ????? (5/5min ? 429)? read-only audit admin? MFA ????????? ??? ?? ???????.
- **Rich text:** allowlist ???? Wagtail (??? HTML ???????)? pytest ???????.
- **Rebuild trigger:** POST-only `/rebuild-trigger/` ?? HMAC-SHA256 + freshness ?5min + `REBUILD_TRIGGER_ENABLED=False` ???????? `scripts/manual-rebuild.sh` fallback.
- **CI:** `.github/workflows/ci-cms.yml` (hosted): uv sync ? check ? makemigrations --check ? ruff ? pytest.
- **Infra candidates (NOT-APPLIED):** `infra/cms/{docker-compose.cms.yml, Dockerfile.cms, Caddyfile.cms.snippet, README.md}` ? deploy ????? ??? ?? ????? ????? ???? (`RISK-0007`) ? Task Spec ???.
- **Verification:** 62 pytest PASS? ruff clean? ???? migration ?? ??????? ADR-0020..0024 ???????? Manifest ? AGENTS.md ? ledgers ??????.
...
- JSON-LD Person/WebSite structured data ??? ???? ????? indexable (LOG-0105)? CI action majors ?? node24 runtime (LOG-0106).

## 2026-08-15 ? R2 first live

- Production P1 live (Language Gateway + bilingual landing)? A1-A5? B3-B5? C1-C3/C5/C6? V1 visual QA (LOG-0064..0104). ????: `docs/plan/RELEASE-P1.md`.
