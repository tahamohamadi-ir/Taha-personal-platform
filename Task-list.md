...
- [x] Playwright browser lifecycle create→publish→public fa/en + full config (retries/trace/video/html). *(LOG-0180 — `qa/e2e/content-lifecycle.spec.ts`; `DEFER-0026` CLOSED)*
...
- [x] Playwright config کامل (retries/trace/video/html reporter) به‌جای spec های خام نگهداری شود. *(§14 S2; LOG-0180 — raw `qa/*.spec.mjs` smoke kept; lifecycle uses `@playwright/test`)*
