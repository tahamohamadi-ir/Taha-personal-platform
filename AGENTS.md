...
**Runtime target (ADR-0027):** one Compose project — `db`, `cms`, `web` (nginx serving Astro HTML, no public Node/React), then `caddy`. Host Caddy remains edge until `DEFER-0031`. Slice 1 cutover is **live** (2026-08-20, PR #50): public HTML via `127.0.0.1:13080`; post-publish use `rebuild-web.sh`. Public pages stay no-JS. Active specs: `docs/plan/cms-origin-and-full-stack-cd-task-spec.md` (Slice 2 CD migrate next), `docs/plan/ADM-6-frontend-wiring-task-spec.md` (PARTIAL). Master plan: `docs/plan/custom-admin-rebuild-fa.md`.
