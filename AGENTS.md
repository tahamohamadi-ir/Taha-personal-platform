...
**P3 code-first gate (2026-08-15, owner-authorized):** `apps/cms/` (Django 5.2.9 / Wagtail 7.4.2 / Django Ninja 1.6.2 / psycopg 3.3.4 on Python 3.12.13) may be developed, tested and CI-verified per `docs/plan/P3-gate-code-first-task-spec.md`. This authorizes code, migrations, tests and the `ci-cms.yml` workflow ONLY. It does NOT authorize: any CMS runtime deployment to staging/production, PostgreSQL provisioning, media upload exposure, public CMS API exposure or contact persistence — those remain blocked until the backup/restore DB-import evidence (`RISK-0003`), owner approval for admin exposure, and the old pre-existing stack decommission. MFA enforcement code exists (`apps/security/mfa.py`, django-otp 1.5.4, 75 pytest PASS) and a deploy Task Spec is written (`docs/plan/P3-cms-deploy-task-spec.md`). `infra/cms/` files are NOT-APPLIED candidates only. Contact persistence, media upload, API/schema exposure and project-specific Docker services remain blocked until a subsequent phase explicitly moves the gate with a complete Task Spec.
