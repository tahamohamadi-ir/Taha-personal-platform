...
Assets/site-redesign/implementation-reference/  next-gen frontend brief only â€” MASTER-SPEC.md, agent-kit/*.json, AGENT-COORDINATION.md, MULTI-AGENT-TASK-LIST.md, ACCEPTANCE-GATES.md (planning/handoff, not live; branch p14c-visual-atlas 7d9b87f)
...
Task-list.md    phased implementation plan (P0-G0 â†’ ATLAS)
```
...
Prerequisites: Node 24 + npm for `apps/web/`; `uv` + Python 3.12 for `apps/cms/`. The default `python` command resolves to a Hermes-owned interpreter and is not the project interpreter â€” always use `uv`.

### Web â€” current runtime (`apps/web/`)

```powershell
npm install        # reproducible install from package-lock.json
npm run check      # astro check (typecheck) â€” 0 errors / 0 warnings
npm run build      # astro build â€” static output in dist/ (default build must NOT contain /_design/)
npm run preview    # serve the built artifact locally
npm audit          # dependency security scan
```

### Web â€” next-generation reference validation
...
# expected: PASS â€” 24 components, 6 templates, 10 asset references, offline Figma builder (ATLAS-00 gate G0)
...
DESIGN_ATLAS=1 npm run build                    # atlas-present build (local only) â€” verify default build stays atlas-free
```

No work packet is executable until the integration lead activates its repository Task Spec (`Assets/site-redesign/implementation-reference/AGENT-COORDINATION.md` Â§6; see [`docs/plan/README.md`](docs/plan/README.md)).
...
- `ci.yml` (web): Node 24; `npm ci` â†’ `npm run check` â†’ `npm run build` â†’ build fingerprint â†’ local-preview smoke via `infra/deploy/smoke.sh` â†’ Playwright mobile-overflow and About-tabs regressions â†’ `npm audit` â†’ artifact completeness and no-secret check â†’ artifact upload.
- `ci-cms.yml` (CMS): `uv sync --python 3.12` â†’ `manage.py check` â†’ `makemigrations --check --dry-run` â†’ `ruff check` â†’ `pytest` â†’ `manage.py test` â†’ `git diff --check` â†’ secret scan.
...
- [PROJECT_MANIFEST.md](PROJECT_MANIFEST.md) â€” product, approved baseline, environments, canonical verified commands.
- [AGENTS.md](AGENTS.md) â€” agent and developer contract, ownership boundaries, current gate (P14 ATLAS).
- [Assets/site-redesign/implementation-reference/README.md](Assets/site-redesign/implementation-reference/README.md) â€” **canonical next-generation frontend brief** (MASTER-SPEC Â§1â€“6, Â§11 + `AGENT-COORDINATION.md` + `MULTI-AGENT-TASK-LIST.md` + `ACCEPTANCE-GATES.md` + `agent-kit/*.json`; branch `p14c-visual-atlas`, commit `7d9b87f`; planning/handoff, not live).
- [Assets/site-redesign/implementation-reference/MASTER-SPEC.md](Assets/site-redesign/implementation-reference/MASTER-SPEC.md) â€” product, architecture, UX, CMS, a11y and delivery contract (Â§11 deliverables, Â§12 non-goals).
- [Assets/site-redesign/implementation-reference/MULTI-AGENT-TASK-LIST.md](Assets/site-redesign/implementation-reference/MULTI-AGENT-TASK-LIST.md) â€” dependency-aware executable work packets ATLAS-00..12 (global constraints + execution board).
- [Assets/site-redesign/implementation-reference/ACCEPTANCE-GATES.md](Assets/site-redesign/implementation-reference/ACCEPTANCE-GATES.md) â€” objective gates G0..G9.
- [Assets/site-redesign/implementation-reference/DOCUMENT-MIGRATION-MAP.md](Assets/site-redesign/implementation-reference/DOCUMENT-MIGRATION-MAP.md) â€” which owner file is reconciled when (ATLAS-12).
- [docs/README.md](docs/README.md) â€” documentation entry point; read order, ownership table, stop conditions (already rewritten for ATLAS).
- [docs/plan/README.md](docs/plan/README.md) â€” **active plan index** â€” ATLAS timeline (ATLAS-00 READY, rest BLOCKED) + archived P1..P11 plans.
- [docs/plan/ATLAS-frontend-rebuild-task-spec.md](docs/plan/ATLAS-frontend-rebuild-task-spec.md) â€” Task Spec for the frontend rebuild from scratch.
- [docs/adr/README.md](docs/adr/README.md) â€” ADR index (ADR-0002..0029, including static-first Astro, versioned artifact deploy, admin security boundary, P3 auth/media/rich-text/rebuild/lifecycle, staging decommission, custom admin replacing Wagtail, runtime target).
- [docs/governance/RELEASE_POLICY.md](docs/governance/RELEASE_POLICY.md) â€” release gate and DoD.
- [docs/governance/DEPLOY_RUNBOOK.md](docs/governance/DEPLOY_RUNBOOK.md) â€” deploy, smoke and rollback procedure.
- [docs/governance/BACKUP_POLICY.md](docs/governance/BACKUP_POLICY.md) and [docs/governance/BACKUP_RUNBOOK.md](docs/governance/BACKUP_RUNBOOK.md) â€” encrypted Google Drive backup policy and runbook.
- [docs/governance/SERVER_ACCESS_RUNBOOK.md](docs/governance/SERVER_ACCESS_RUNBOOK.md) â€” key-only non-root SSH access.
- [docs/governance/DOCUMENTATION_POLICY.md](docs/governance/DOCUMENTATION_POLICY.md) â€” documentation contract.
- [docs/status/](docs/status/) â€” ledgers: [WORK_LOG.md](docs/status/WORK_LOG.md), [RISK_REGISTER.md](docs/status/RISK_REGISTER.md), [deferred-validation.md](docs/status/deferred-validation.md), [TECH_DEBT.md](docs/status/TECH_DEBT.md), [known-issues.md](docs/status/known-issues.md), [CHANGELOG.md](docs/status/CHANGELOG.md), [BACKLOG.md](docs/status/BACKLOG.md).
- [docs/contracts/](docs/contracts/) â€” `IA-CONTRACT.md` (routing/URLs) + `DESIGN-CONTRACT.md` (visual/tokens, current runtime).
- [docs/templates/TASK_SPEC_TEMPLATE.md](docs/templates/TASK_SPEC_TEMPLATE.md) â€” task specification template.
- [docs/design.md](docs/design.md) â€” **history / deep reference** (superseded by `MASTER-SPEC.md` + `agent-kit/*.json`).
- [reDesign_plan.md](reDesign_plan.md) â€” visual redesign draft; executed via ATLAS tokens/components. Its Â§12â€“13 hard limits are **outranked by `MASTER-SPEC.md`** for tokens when they conflict.
- [Task-list.md](Task-list.md) â€” phased implementation plan and progress snapshot (P0-G0 â†’ ATLAS).

## Status snapshot

- Live (2026-08-17/18/23): static site at https://tahamohamadi.ir (Language Gateway, P4â€“P6 + P8 catalogs + `/search/` Pagefind) plus CMS Compose `taha-cms` (`116c241`, `content.0013` + `content.0014`); public published-only `/api/` + `/media/` live (`DEFER-0017` CLOSED). Custom admin SPA at `/admin/` (ADM-1 cutover, LOG-0163); Django staff HTML at `/staff/` (Wagtail removed, `DEBT-0003` CLOSED / LOG-0193). Evidence: [WORK_LOG.md](docs/status/WORK_LOG.md).
- **P14 ATLAS â€” next-generation frontend: NOT STARTED.** Reference package `Assets/site-redesign/implementation-reference/` validated (commit `7d9b87f`); no token, component, template, or route from it is live. Existing `apps/web` static remains current runtime until ATLAS packets are accepted and merged. Plan index: [docs/plan/README.md](docs/plan/README.md).
...
Phased plan in [Task-list.md](Task-list.md) and [docs/plan/README.md](docs/plan/README.md): P0-G0 gate passed, static R1 spine and R2 first live done, P2 About/CV live, P3â€“P6 CMS/public API live, custom admin rebuild authorized (ADR-0026, phases ADM-0..ADM-6 in Task-list Â§17; Wagtail removal done), P8â€“P11 catalogs/search and AI/semantic deferred per plan, **P14 ATLAS next-generation frontend brief is canonical (planning, not live) â€” rebuild from scratch in `apps/web/` as ATLAS-00..12 packets** (tokens â†’ primitives â†’ shell/gateway â†’ content components â†’ 6 templates â†’ local-only Visual Atlas â†’ route-family adoption â†’ CMS audit â†’ CMS implementation â†’ graph Phase 1 â†’ QA â†’ documentation reconciliation). Owner-filtered queue: [docs/status/BACKLOG.md](docs/status/BACKLOG.md).
