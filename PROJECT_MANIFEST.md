...
| Backup | Encrypted restic repository through rclone on Google Drive | restic 0.18.1 and Ubuntu rclone 1.60.1 build installed; Google Drive OAuth, target-folder access, encrypted repository, PostgreSQL/media/config snapshots and `restic check` verified; scheduled job and staging restore remain |
