...
apps/cms/               Django + Django Ninja only (Wagtail removed, DEBT-0003 CLOSED); admin SPA lives in apps/admin (independent since ADR-0032) — NOT inside cms; admin SPA served at /admin/ (ADR-0032) — historic tree note removed:dmin/ + Django staff HTML at /staff/
...
- **Frontend/admin separation is invariant (ADR-0026):** `apps/web` (public Astro frontend, including local-only Visual Atlas) and `apps/cms` + `apps/admin` remain **separate projects, separate builds, separate routes**. No shared writable worktree, no merged bundle — must not be violated.
...
| Dark mode | Not in R2; full dark mode deferred with ID | NOT USED IN R2 — **default theme after ATLAS-01 = `system` (owner decision 2026-08-26)**; — now design-target in `tokens.json`; implementation is ATLAS-01 (G1 gate) |
